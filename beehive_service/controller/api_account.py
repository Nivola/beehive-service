...
class ApiAccount(AuthorityApiObject):
    ...
    def __init__(self, *args, **kvargs):
        ...
        if self.model is not None:
            ...
            self.email_support_link = self.email_support_link
    ...
    def __repr__(self):
        return '<%s id=%s objid=%s name=%s>' % ('ApiAccount', self.oid, self.objid, self.name)
    ...
    def get_username_roles(self, *args, **kvargs):
        ...
        for role_template in self.role_templates.values():
            ...
            # users = []
            roles = self.api_client.admin_request('auth', '/v1.0/nas/roles', 'GET',
                                                  data=urlencode({'names': role_name})).get('roles')
            for role in roles:
                ...
                for user in users:
                    ...
                    user.update({'account_name': self.name, 'email': self.email,
                                 'account_status': self.status, 'contact': self.contact})
    ...
    def delete(self, soft=False, **kvargs):
        ...
        if self.has_service('NetworkGateway') is True:
            raise ApiManagerError('account %s has an active internet gateway. Remove before it delete account' %
                                  self.uuid)
    ...
    def has_service(self, plugintype):
        services, tot = self.controller.get_service_type_plugins(account_id=self.oid, plugintype=plugintype,
                                                                 with_perm_tag=False)
        if tot > 0:
            return True
        return False
    ...
    def get_service_index(self):
        service_insts, tot = self.controller.get_paginated_service_instances(account_id=self.oid, size=-1,
                                                                             with_perm_tag=False, filter_expired=False)
    ...
    # Definitions
    #
    def can_instantiate(self, definition: ApiServiceDefinition = None, definition_id: Union[int, str] = None)\
            -> Tuple[bool, str]:
        ...
        if definition is not None:
            ...
            accsdlist, tot = self.controller.get_account_service_defintions(
                account_id=self.model.id, service_definition_id=definition.model.id, actice=1)
            if tot == 0:
    ...
    def get_default_services_description(self):
        ...
        def notin(slist, srv):
            if srv is None:
                return False
            srv_ty = srv.get(u"type", "--")
            srv_nm = srv.get(u"name", "--")
            for item in slist:
                item_ty = item.get(u"type", "--")
                item_nm = item.get(u"name", "--")
    ...
    def get_capabilities_list(self):
        ...
        if self.model:
            ...
            for c in capabilities:
                ...
                status = c.get('status')
                if service_created != service_required and status in [SrvStatusType.ACTIVE, SrvStatusType.ERROR]:
    ...
    def get_report_costconsume(self, year_month, start_date, end_date, report_mode, *args, **kvargs):
        ...
        :return:
        """
        res_report = self.controller._format_report_costcomsume([self.oid], year_month, start_date, end_date,
                                                                report_mode, *args, **kvargs)
        ...
        referent = '' if self.model.division.organization.referent is None else \
            self.model.division.organization.referent
