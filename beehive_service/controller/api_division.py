...
class ApiDivision(AuthorityApiObject):
    ...
    def __repr__(self):
        return '<%s id=%s objid=%s name=%s>' % ('ApiDivision', self.oid, self.objid, self.name)
    ...
    def get_cost_by_period(self, start_period, end_period=None,
        ...
        return self.manager.get_cost_by_authority_on_period(period_start=start_period, period_end=end_period,
                            division_id=self.oid, plugin_name=plugin_name, reported=reported)
    ...
    def get_report_costconsume (self, year_month, start_date, end_date, report_mode, *args, **kvargs):
        ...
        accounts, total = self.controller.get_accounts(division_id=self.oid, active=True,
        ...
        res_credit_summary = self.controller._format_report_credit_summary(account_list_id, [self.oid], year_month, start_date, end_date)
        res_credit_composition = self.controller._format_report_credit_composition([self.oid], year_month, start_date, end_date)
