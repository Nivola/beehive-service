...
class ServiceController(ApiController):
    ...
    def get_account_idx(self, *args, **kvargs) -> Dict[str, ApiAccount]:
        ...
        ret: Dict[str, ApiAccount] = self.get_entities_idx(
            ApiAccount,
    ...
    def get_service_tags_idx(self, *args, **kvargs) -> Dict[str, ApiServiceTag]:
        """Get service indexed by id"""
        return self.manager.get_service_tags_idx(*args, **kvargs)
    ...
    def get_service_definition_idx(self, plugintype: str) -> Dict[str, ApiServiceDefinition]:
        ...
        ret: Dict[str, ApiServiceDefinition] = self.get_entities_idx(
            ApiServiceDefinition,
            self.manager.get_paginated_service_definitions,
    ...
    def get_service_instance_config_idx(self, *args, **kvargs) -> Dict[str, ApiServiceInstanceConfig]:
        ...
        ret: Dict[str, ApiServiceInstanceConfig] = self.get_entities_idx(
            ApiServiceInstanceConfig,
            self.manager.get_paginated_service_instance_configs,
            *args,
            **kvargs,
        )
        return ret
    ...
    def get_jobs(self, *args, **kvargs):
        ...
        res, total = self.get_paginated_entities(
            ApiServiceJob, recuperaServiceJobs, customize=customize, *args, **kvargs
        )
        return res, total

    def get_task_intervals(self, execution_date=None, metric_num=None, task=None, *args, **kvargs):
        tasks = self.manager.get_task_intervals(
    ...
    def get_account_service_defintion(self, oid: Union[int, str], for_update=False) -> ApiAccountServiceDefinition:
        ...
        :raises ApiManagerError: raise :class:`ApiManagerError`
        """
        adef: ApiAccountServiceDefinition = self.get_entity(
            ApiAccountServiceDefinition,
            AccountServiceDefinition,
            oid,
    ...
    def add_account_service_definition(
        self,
        account_id: int,
        service_definition_id: int,
        name: str = None,
        account: ApiAccount = None,
        servicedefinition: ApiServiceDefinition = None,
    ):
        ...
        self.logger.debug(
            f'check_authorization for "{ApiAccountServiceDefinition.objtype}", '
            f'"{ApiAccountServiceDefinition.objdef}", "insert"'
        )
        self.check_authorization(
            ApiAccountServiceDefinition.objtype,
            ApiAccountServiceDefinition.objdef,
            objid,
            "insert",
        )
        ...
        try:
            ...
            ApiAccountServiceDefinition(
                ...
                desc=res.desc,
                model=res,
            ).register_object([objid], desc=name)
    ...
    def get_service_job_schedule(self, oid: Union[int, str], for_update: bool = False):
        ...
        :raises ApiManagerError: raise :class:`ApiManagerError`
        """
        srv_js: ApiServiceJobSchedule = self.get_entity(
            ApiServiceJobSchedule, ServiceJobSchedule, oid, for_update=for_update
        )
        return srv_js
    ...
    def get_service_price_list_by_account(self, account_id, period_date):
        ...
        if listino is not None:
            ...
            if account is not None:
                ...
                if listino is not None:
                    self.logger.debug(
                        "account %s con listino = %s da division %s" % (account_id, listino, account.division_id)
                    )
                    price_list_id = listino.price_list_id
                else:
                    raise ApiManagerWarning(
                        "Price List not found for Account %s and Division %s" % (account_id, account.division_id)
                    )
            else:
    ...
    def get_service_price_metrics(self, *args, **kvargs):
        ...
        def recuperaServicePriceMetrics(*args, **kvargs):
            ...
            entities, total = self.manager.get_paginated_service_price_metrics(
                price_list_id=srv_price_list.oid, *args, **kvargs
            )
        ...
        res, total = self.get_paginated_entities(
            ApiServicePriceMetric,
            recuperaServicePriceMetrics,
            customize=customize,
            *args,
            **kvargs,
        )
    ...
    def add_service_price_metric(
        ...
        # check authorization
        self.check_authorization(
            ApiServicePriceMetric.objtype,
            ApiServicePriceMetric.objdef,
            price_list.objid,
            "insert",
        )
        try:
    ...
    def get_service_metrics(self, metric_type=None, *args, **kvargs):
        ...
        res, total = self.manager.get_paginated_service_metrics(
            metric_type_id=metric_type_id, with_perm_tag=False, *args, **kvargs
        )

        return res, total

    def add_service_metrics(self, data: dict) -> int:
        """add_service_metrics
        Implemet the add metrics use case
        this implementation write metrics on database and on eleastic search.
        Can now have an account id/uuid in order of pre loading information abaout account services.
        Recive a single metric definition or a list of metrics for implementing a bulk insert of metrics.

        Args:
            data (dict):  data recived from view
        account:[int, str, None]  the account id or uuid
        metric = matrice definition or list of metric definition

        Raises:
            ApiManagerError: if som trosuction fail
        ...
        account_id: Union[int, str, None] = data.get("account", None)
        metrics = data.get("metrics", None)
        metric = data.get("metric", None)

        def get_job(job: dict) -> int:
            if job.get("dummy", None) is None:
                job["dummy"] = dao.get_dummy_process_id()
            return job["dummy"]

        def newmetric(name: str) -> int:
            try:
                # create objid reference
                objid = id_gen()

                mt = ServiceMetricType(
                    objid,
                    name=name,
                    metric_type=name,
                    group_name=name,
                ...
                metricsbyname[name] = res.id
                metricsbyid[str(res.id)] = name
                return res.id
        ...
        def reindex(idx: dict, intvalue: bool = True) -> dict:
            """reindex  take a

            Args:
                idx dict: dictionary used as index
                intvalue (bool, optional): _description_. Defaults to True.
        ...
        def normalize(metricitem: dict) -> dict:
            si_oid = metricitem.get("service_instance_oid", None)
            metric = {
                "account_id": 0,
                "account_uuid": 0,
                "metric_type": metricitem.get("metric_type", None),
                "value": metricitem.get("value", None),
                "metric_type_id": metricitem.get("metric_type_id", None),
                "metric_type": metricitem.get("metric_type", None),
                "metric_num": metricitem.get("metric_num", 0),
                "service_instance_id": metricitem.get("service_instance_oid", None),
                "service_instance_uuid": metricitem.get("service_instance_oid", None),
                "resource_uuid": metricitem.get("resource_uuid", None),
                # "job_id": metricitem.get("job_id", None),
                "job_id": get_job(dummyjob),
                "creation_date": metricitem.get("creation_date", None),
            }
            # sanify creation_date
            try:
                cd = metric["creation_date"]
                if type(cd) == datetime:
                    metric["creation_date"] = cd
                elif type(cd) == str:
                    metric["creation_date"] = parse_date(cd)
                else:
                    metric["creation_date"] = datetime.now()
            except Exception as ex:
                self.logger.error(ex)
                metric["creation_date"] = datetime.now()
            mt_name = None
            mt_id = None
            # sanify metric_type metric_type_id
            if metric["metric_type"] is not None:
                mt_name = metric["metric_type"]
                mt_id = metricsbyname.get(mt_name, None)
                if mt_id is None:
                    _, mt_id = dao.get_service_metric_types_info(name=mt_name)
                if mt_id is None:
                    mt_id = newmetric(mt_name)
            else:
                ## by validation rule assume metric_type_id is set
                mt_id = metric["metric_type_id"]
                mt_name = metricsbyid.get(str(mt_id), None)
                if mt_name is None:
                    mt_name, mt_id = dao.get_service_metric_types_info(id=mt_id)
            metric["metric_type_id"] = mt_id
            metric["metric_type"] = mt_name

            # sanify resource_uuid service_instance_oid
            info = None

            # get service info for dictionary index if collected or by query
            if metric["resource_uuid"] is not None:
                # get service by resource_uuid check if pre-collectetd by account otherwise  get fro db
                info = servinfo.get(metric["resource_uuid"], None)
                if info is None:
                    info = dao.get_service_info(resource_uuid=metric["resource_uuid"])
            else:
                # serche service
                if si_oid is not None:
                    # get service info by uuid check if pre-collectetd by account otherwise  get fro db
                    for k in servinfo.keys():
                        i = servinfo[k]
                        if i["service_id"] == si_oid or i["service_uuid"] == si_oid:
                            info = i
                            break
                    if info is None:
                        if is_uuid(si_oid):
                            info = dao.get_service_info(uuid=si_oid)
                        else:
                            info = dao.get_service_info(id=si_oid)
                ## by validation rule assume service_instance_oid is set
            if info is None:
                return metric

            metric["resource_uuid"] = info.get("resource_uuid", 0)
            metric["service_instance_id"] = info.get("service_id", 0)
            metric["service_instance_uuid"] = info.get("service_uuid", 0)
            metric["account_id"] = info.get("account_id", 0)
            metric["account_uuid"] = info.get("account_uuid", "")
            return metric

        def dbmetric(metricitem: dict) -> int:
            srv_m = ServiceMetric(
                value=metricitem["value"],
                metric_type_id=metricitem["metric_type_id"],
                metric_num=0,
                service_instance_id=metricitem["service_instance_id"],
                job_id=metricitem["job_id"],
                resource_uuid=metricitem["resource_uuid"],
                creation_date=metricitem["creation_date"],
            )
            self.logger.warn("Metric=%s" % srv_m)
            res = self.manager.add(srv_m)
            return res.id

        def elasticmetric(metricitem: dict):
            """add metric to elastic index

            Args:
                metricitem (dict):  the metric to add created by normalize function
            """
            try:
                from elasticsearch import Elasticsearch

                prefix = "cmp-metrics-instantconsume"
                index = "%s-%s" % (prefix, datetime.now().date().strftime("%Y.%m.%d"))

                elastic: Elasticsearch = self.api_manager.elasticsearch
                resp = elastic.index(
                    index=index,
                    document={
                        "metric_type": metricitem.get("metric_type", None),
                        ...
                        "metric_type": metricitem.get("metric_type", None),
                        "metric_num": metricitem.get("metric_num", 0),
                        "service_instance_id": metricitem.get("service_instance_id", None),
                        "service_instance_uuid": metricitem.get("service_instance_uuid", None),
                        "resource_uuid": metricitem.get("resource_uuid", None),
                        "creation_date": format_date(metricitem.get("creation_date", datetime.now())),
                    },
                )
                self.logger.debug("SENT ELASTICSEARCH " + index + " " + str(resp))

            except Exception as ex:
                print("EXCEPTION ", ex)
                self.logger.error(ex)
                # raise NotImplementedError("Elastimetric not jet implemented")
            if True:
                return

        # check authorization only administrator can insert metrics
        # do all on all accounts
        self.check_authorization(
            ApiAccount.objtype,
            ApiAccount.objdef,
            "*//*//*",
        ...
        if type(metrics) == list:
            if len(metrics) > 1 and account_id is not None:
                servinfo = dao.get_service_info_for_account(account_id)
                metricsbyname: dict = dao.get_service_metric_types_dict(byname=True)
                metricsbyid: dict = reindex(metricsbyname, intvalue=True)
        ...
        if type(metric) == dict:
            norma = normalize(metric)
            ret = dbmetric(norma)
            elasticmetric(norma)

        return ret

    def add_service_metric(
        self,
        value,
        metric_type_id,
        metric_num,
        service_instance_oid,
        job_id,
        creation_date,
    ):
        """add_service_metric
        deprecated add metric for inserting single

        Args:
            value (_type_): _description_
            metric_type_id (_type_): _description_
            metric_num (_type_): _description_
            service_instance_oid (_type_): _description_
            job_id (_type_): _description_
            creation_date (_type_): _description_

        Raises:
            ApiManagerWarning: _description_
    ...
    def delete_service_metric(
        ...
        try:
            if metric_oid is not None:
                ...
                res = self.manager.delete_service_metric(
                    metric_type_id=metric_type_id,
                    metric_num=metric_num,
                    service_instance_id=instance_id,
                    job_id=job_id,
    ...
    def get_service_instantconsume(self, oid: Union[int, str], request_date, *args, **kvargs):
        ...
        resp = {"account_id": account.oid, "request_date": request_date, "metrics": []}
    ...
    def add_service_metric_type_limit(self, parent_id, item, *args, **kvargs):
        ...
        if fk_mt.metric_type != MetricType.CONSUME.name:
            raise ApiManagerWarning(
                "check limit parameters: it is referered a wrong metric type %s %s" % (fk_mt.oid, fk_mt.metric_type)
            )

        new_mtl = ServiceMetricTypeLimit(
            name=item.get("name"),
            parent_id=parent_mt.oid,
            metric_type_id=fk_mt.oid,
    ...
    def get_service_plugin_type(self, authorize=True, *args, **kvargs):
        ...
        res = []
        for item in items:
            res.append({"id": item.id, "name": item.name_type, "objclass": item.objclass})
    ...
    def get_service_type(self, oid):
        ...
        :raises ApiManagerError: raise :class:`ApiManagerError`
        """
        srv_type = self.get_entity(ApiServiceType, ServiceType, oid)
        return srv_type
    ...
    def get_service_types(self, *args, **kvargs):
        ...
        res, total = self.get_paginated_entities(
            ApiServiceType, recuperaServiceTypes, customize=customize, *args, **kvargs
        )
        ...
        try:
            ...
            for inst in insts:
                ...
                obj = inst_class(
                    self,
                    oid=inst_type.id,
                    objid=inst_type.objid,
                    name=inst_type.name,
                    active=inst_type.active,
                    desc=inst_type.desc,
                    model=inst_type,
                )

                inst_obj = ApiServiceInstance(
                    self,
                    oid=inst.id,
                    objid=inst.objid,
                    name=inst.name,
    ...
    def add_service_type_plugin(
        ...
        **kvargs,
        ...
        if instance_config is None:
            instance_config = {}
        # get definition
        service_definition = self.get_service_def(service_definition_id, authorize=False)
        if service_definition.is_active() is False:
            raise ApiManagerWarning(f"Service definition {service_definition.uuid} is not in ACTIVE state")
        # get account
        if account is None:
            account = self.get_account(account_id)
        ...
        if len(name) > name_max_length:
            raise ApiManagerError("Service name %s is too long. Maxsize is %s" % (name, name_max_length))
        ...
        try:
            ...
            inst = self.add_service_instance(
                name=name,
                desc=desc,
                service_def_id=service_definition.oid,
                status=SrvStatusType.DRAFT,
                account=account,
    ...
    @trace(entity="ApiServiceInstance", op="insert")
    def import_service_type_plugin(
        self,
        service_definition_id,
        account_id,
        name=None,
        ...
        **kvargs,
        ...
        try:
            # post import service using sync method
            inst.update_status(SrvStatusType.BUILDING)
    ...
    def get_service_process(self, oid):
        ...
        srv_cost_cpram = self.get_entity(ApiServiceProcess, ServiceProcess, oid)
        return srv_cost_cpram
    ...
    def add_service_process(
        ...
        self.check_authorization(ApiServiceType.objtype, ApiServiceType.objdef, servicetype.objid, "update")
        try:
            # create reference
            objid = id_gen(parent_id=servicetype.objid)
            srv_cp = ServiceProcess(
                objid,
                name,
                servicetype.oid,
                method_key,
    ...
    def get_service_cfg(self, oid):
        ...
        srv_cfg = self.get_entity(ApiServiceConfig, ServiceConfig, oid)
        return srv_cfg
    ...
    def add_service_cfg(
        ...
        # check authorization
        self.check_authorization(
            ApiServiceConfig.objtype,
            ApiServiceConfig.objdef,
            service_def.objid,
            "insert",
        )
        try:
            ...
            srv_type = ServiceConfig(
                objid=objid,
                name=name,
                desc=desc,
                service_definition_id=service_def.oid,
                params=params,
    ...
    def get_service_def(self, oid: Union[int, str], authorize=False) -> ApiServiceDefinition:
        ...
        :raises ApiManagerError: raise :class:`ApiManagerError`
        """
        srv = self.get_entity(ApiServiceDefinition, ServiceDefinition, oid, authorize=False)
        return srv
    ...
    def get_default_service_def(self, plugintype):
        ...
        if len(defs) != 1:
            self.logger.error(
                "Service definition default for type %s does not exist" % plugintype,
                exc_info=True,
            )
            raise ApiManagerError(
                "Service definition default for type %s does not exist" % plugintype,
    ...
    def check_service_definition(
        self,
        service_definition_id: Union[int, str],
        service_definition_name: str = "ServiceDefinition",
    ) -> ApiServiceDefinition:
        ...
        service_definition = self.get_service_def(service_definition_id)
        if service_definition.is_active() is False:
            raise ApiManagerWarning("%s %s is not in ACTIVE state" % (service_definition_name, service_definition.uuid))
    ...
    def add_service_def(
        ...
        if len(defs) > 0:
            self.logger.error(
                "Service definition %s for type %s already exists" % (name, serviceType.name),
                exc_info=True,
            )
            raise ApiManagerError(
                "Service definition %s for type %s already exists" % (name, serviceType.name),
        ...
        if is_default is True:
            ...
            if len(defs) > 0:
                self.logger.error(
                    "Service definition default for type %s already exists" % serviceType.name,
                    exc_info=True,
                )
                raise ApiManagerError(
                    "Service definition default for type %s already exists" % serviceType.name,
        ...
        # check authorization
        self.check_authorization(
            ApiServiceDefinition.objtype,
            ApiServiceDefinition.objdef,
            serviceType.objid,
            "insert",
        )
        try:
    ...
    def createInstanceHierachy(
        ...
        **kvargs,
        ...
        if serviceDefRoot is not None:
            ...
            parent = self.add_service_instance(
                name=instName,
                desc=instDesc,
                service_def_id=serviceDefRoot.id,
                status=SrvStatusType.DRAFT,
                account=account,
                bpmn_process_id=None,
                active=True,
            ...
            for linkDef in serviceDefRoot.linkChildren:
                ...
                child = self.createInstanceHierachy(
                    linkDef.end_service_id,
                    None,
                    service_def=link_end_service,
                    params_resource=params_resource,
                    account=account,
                )
                # insert Link
                linkInstName = "lnk_%s_%s" % (parent.oid, child.oid)
                self.add_service_instlink(
                    linkInstName,
                    parent.oid,
                    child.oid,
                    priority=linkDef.priority,
                    desc=linkInstName,
                    attributes=linkDef.attributes,
                )
            return parent
        else:
    ...
    def add_service_instance(
        ...
        insts, tot = self.get_paginated_service_instances(
            name=name, account_id=account.oid, filter_expired=False, authorize=False
        )
        if tot > 0:
            raise QueryError(
                "Service instance %s already exists in account %s" % (name, account.uuid),
                code=409,
            )
    ...
    def delete_service_instance(self, srv_inst, data, recursive_delete=True, batch=True):
        ...
        if srv_inst.model.linkChildren is not None and srv_inst.model.linkChildren.count() > 0:
            if not recursive_delete:
                self.logger.warn(
                    "The instance %s has children. Force deletion with parameter " '"recursive" to true.' % srv_inst.oid
                )
                raise ApiManagerWarning(
                    "The instance %s has children. Force deletion with parameter " '"recursive" to true.' % srv_inst.oid
                )
    ...
    def add_service_instance_cfg(self, name="", desc=None, service_instance=None, json_cfg=None, active=True):
        ...
        # check authorization
        self.check_authorization(
            ApiServiceInstanceConfig.objtype,
            ApiServiceInstanceConfig.objdef,
            service_instance.objid,
            "insert",
        )
        try:
            ...
            cfg = ServiceInstanceConfig(
                objid,
                name,
                service_instance.oid,
                json_cfg=json_cfg,
                desc=desc,
                active=active,
            )
            res = self.manager.add(cfg)
            api_cfg = ApiServiceInstanceConfig(
                self,
                oid=cfg.id,
                objid=cfg.objid,
                name=cfg.name,
    ...
    def add_service_instlink(
        ...
        self.check_authorization(
            ApiServiceLinkInst.objtype,
            ApiServiceLinkInst.objdef,
            srv_instance_start.objid,
            "insert",
        )
        try:
            ...
            srv_istlink = ServiceLinkInstance(
                objid,
                name,
                srv_instance_start.oid,
                srv_instance_end.oid,
                priority=priority,
                desc=desc,
    ...
    def get_service_instlink(self, oid, authorize=True):
        ...
        :raises ApiManagerError: raise :class:`ApiManagerError`
        """
        srv_instlink = ApiController.get_entity(self, ApiServiceLinkInst, ServiceLinkInstance, oid, authorize)
        return srv_instlink
    ...
    def list_service_deflink(self, *args, **kvargs):
        ...
        def get_entities(*args, **kvargs):
            entities, total = self.manager.get_service_links(ServiceLinkDef, *args, **kvargs)
        ...
        res, total = self.get_paginated_entities(ApiServiceLinkDef, get_entities, customize=customize, *args, **kvargs)
        return res, total
    ...
    def get_service_deflink(self, oid, authorize=True):
        ...
        :raises ApiManagerError: raise :class:`ApiManagerError`
        """
        srv_deflink = ApiController.get_entity(self, ApiServiceLinkDef, ServiceLinkDef, oid, authorize)
        return srv_deflink

    @trace(entity="ApiServiceLinkDef", op="insert")
    def add_service_deflink(
        self,
        name,
        start_service_id,
        end_service_id,
    ...
    def get_organizations(self, *args, **kvargs):
        ...
        res, total = self.get_paginated_entities(ApiOrganization, get_entities, customize=customize, *args, **kvargs)
        return res, total
    ...
    def add_division(
        ...
        **kvargs,
        ...
        if tot > 0:
            raise QueryError(
                "Division %s already exists in organization %s" % (name, organization_id),
                code=409,
            )
    ...
    def get_divisions(self, *args, **kvargs):
        ...
        res, total = self.get_paginated_entities(ApiDivision, get_entities, customize=customize, *args, **kvargs)
        return res, total
    ...
    def add_capability(self, name, desc="", version="1.0", *args, **kvargs):
        ...
        try:
            ...
            api_capability = ApiAccountCapability(
                self,
                oid=capability.id,
                objid=capability.objid,
    ...
    def get_capabilities(self, *args, **kvargs):
        ...
        res, total = self.get_paginated_entities(
            ApiAccountCapability, get_entities, customize=customize, *args, **kvargs
        )
        return res, total
    ...
    def add_account(
        ...
        **kvargs,
        ...
        if tot > 0:
            raise QueryError(
                "Account %s already exists in division %s" % (name, division_id),
                code=409,
            )
    ...
    def set_account_applied_bundle(self, oid, bundles=None):
        ...
        try:
            for bundle in bundles:
                ...
                applied_bundles = self.manager.get_applied_bundle_list(
                    account_id=account.oid,
                ...
                new_applied_bundle = AppliedBundle(
                    account_id=account.oid,
                    metric_type_id=srv_mt.oid,
                    start_date=start_date,
                    end_date=end_date,
    ...
    def update_account_applied_bundle(self, oid, data, *args, **kvargs):
        ...
        # check authorization
        self.check_authorization(ApiAccount.objtype, ApiAccount.objdef, account.objid, "update")
    ...
    def unset_account_applied_bundle(self, oid, bundle_id, all=False, *args, **kvargs):
        ...
        account = self.get_entity(ApiAccount, Account, oid)
        # check authorization
        self.check_authorization(ApiAccount.objtype, ApiAccount.objdef, account.objid, "delete")
        try:
    ...
    def get_report_cost_monthly_by_accounts(self, account_ids, start_date, end_date, plugin_name=None, *args, **kvargs):
        ...
        try:
            rcs = self.manager.get_report_cost_monthly_by_accounts(
                account_id_list,
                start_date=start_date,
                end_date=end_date,
    ...
    def get_cost_by_account_on_period(
        self,
        oid,
        start_period,
        end_period=None,
    ...
    def add_agreement(
        ...
        desc="",
        amount=0.00,
        agreement_date_start=None,
        agreement_date_end=None,
        *args,
        **kvargs,
    ):
        """ """
        ...
        division = self.get_division(division_id)
        division_objid = division.objid
        self.check_authorization(ApiDivision.objtype, ApiDivision.objdef, division_objid, "use")
        ...
        self.check_authorization(ApiWallet.objtype, ApiWallet.objdef, wallet_objid, "use")
        try:
            # create  Agreement
            ...
            agreement = self.manager.add_agreement(
                ...
                agreement_date_end=agreement_date_end,
                active=True,
            )
    ...
    def get_agreements(self, *args, **kvargs):
        ...
        res, total = self.get_paginated_entities(ApiAgreement, get_entities, customize=customize, *args, **kvargs)
        return res, total
    ...
    def add_wallet(
        ...
        self.check_authorization(ApiWallet.objtype, ApiWallet.objdef, division_objid, "insert")
        try:
            # create  Wallet
    ...
    def get_wallet(self, oid, authorize=True):
        ...
        :raises ApiManagerError: raise :class:`ApiManagerError`
        """
        wallet = ApiController.get_entity(self, ApiWallet, Wallet, oid, authorize)
        return wallet
    ...
    def get_wallets(self, *args, **kvargs):
        ...
        res, total = self.get_paginated_entities(ApiWallet, get_entities, customize=customize, *args, **kvargs)
        return res, total
    ...
    def get_tags_occurrences(self, *args, **kvargs):
        ...
        res, total = ApiController.get_paginated_entities(
            self, ApiServiceTag, get_entities, customize=customize, *args, **kvargs
        )
        return res, total
    ...
    def add_link(
        ...
        if operation.authorize is True:
            self.check_authorization(
                ApiServiceInstanceLink.objtype,
                ApiServiceInstanceLink.objdef,
                account.objid,
                "insert",
            )
    ...
    def get_user_roles(self, user_name=None, group_name=None, group_id_list=None, size=10):
        ...
        res = self.api_client.admin_request("auth", "/v1.0/nas/roles", "get", data=urlencode(data))
    ...
    def get_user_groups(self, user_name, size=10):
        ...
        data = {"user": user_name, "size": size, "active": True}

        res = self.api_client.admin_request("auth", "/v1.0/nas/groups", "get", data=urlencode(data))

        return res

    ###### Service instant consume ########################
    def _format_get_service_instant_consume(self, *args, **kvargs):
        service_plugin_types = self.get_service_plugin_type(
            authorize=False,
            category="CONTAINER",
    ...
    def get_service_instant_consume_by_nivola(self, *args, **kvargs):
        ...
        if len(orgs_list_id) > 0:
            ...
            divs, tot_divs = self.get_divisions(
                organization_id_list=orgs_list_id,
                active=True,
                filter_expired=False,
                size=0,
            )
        ...
        if len(div_list_id) > 0:
            ...
            accounts, tot_accs = self.get_accounts(
                divisions_list_id=div_list_id, active=True, filter_expired=False, size=0
            )
    ...
    def get_service_instant_consume_by_organization(self, oid=None, *args, **kvargs):
        ...
        divs, tot_divs = self.get_divisions(
            organization_id=self.get_organization(oid).oid,
            active=True,
            filter_expired=False,
            size=0,
        )
    ...
    def get_service_instant_consume_by_division(self, oid, *args, **kvargs):
        """ """
        # get active account for division
        accounts, total = self.get_accounts(
            division_id=self.get_division(oid).oid,
            active=True,
            filter_expired=False,
            size=0,
        )
    ...
    def get_paginated_service_instant_consumes(
        ...
        **kvargs,
        ...
        res, total = self.manager.get_paginated_service_instant_consumes(
            id=id,
            organization_id=organization_id,
    ...
    def _format_report_costcomsume(
        self,
        account_ids,
        year_month,
        start_date,
        ...
        **kvargs,
        ...
        plugin_containers = self.manager.get_plugin_type_by_account(
            account_ids=account_ids, category=SrvPluginTypeCategory.CONTAINER
        )
        ...
        for plugin_name in plugin_containers:
            service = {"name": plugin_name, "plugin_name": plugin_name}
            monthly_cost = self.get_report_cost_monthly_by_accounts(
                account_ids,
        ...
        res = {
            ...
            "period": {
                "start_date": format_date(start_period),
                "end_date": format_date(end_period),
            },
            "amount": imp_periodo,
            "services": services,
        }
    ...
    def get_report_costconsume_bynivola(self, year_month, start_date, end_date, report_mode, *args, **kvargs):
        ...
        res_credit_summary = self._format_report_credit_summary(
    ...
    def get_cost_by_nivola_on_period(
        self,
        start_period,
        end_period=None,
        plugin_name=None,
