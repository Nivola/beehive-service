...
from beehive_service.model.base import Base, SrvStatusType, ConfParamType, ApiBusinessObject
...
class ServiceDefinition(ApiBusinessObject, Base):
    ...
    linkParent: ServiceLinkDef = relationship('ServiceLinkDef',
          ...
          back_populates="end_service", single_parent=True)
    linkChildren: List[ServiceLinkDef] = relationship("ServiceLinkDef",
          primaryjoin="and_((ServiceLinkDef.start_service_id==ServiceDefinition.id),"
          "or_(ServiceLinkDef.expiry_date==None, "
          "ServiceLinkDef.expiry_date>=func.now()))",
          back_populates="start_service", lazy="dynamic")

    config_params: List[ServiceConfig] = relationship("ServiceConfig", back_populates="service_definition",
    ...
    def __init__(self, objid, name, desc, service_type_id, active=True, version='1.0', status=SrvStatusType.RELEASED,
                 is_default=False):
    ...
    def __repr__(self):
        return "<Model:ServiceDefinition(id:%s, objid:%s, name:%s)>" % (self.id, self.objid, self.name)
...
class ServiceConfig(ApiBusinessObject, Base):
    ...
    service_definition = relationship(
        "ServiceDefinition", back_populates="config_params")
    # Config params in json format
    params = Column(TextDictType(), default={})
    # JSON, YAML, ... (Da copire)
    params_type = Column(String(20))

    def __init__(self, objid, name, service_definition_id, params='{}', params_type=ConfParamType.JSON, desc='',
    ...
    def __repr__(self):
        return "<Model:ServiceConfig(id:%s, srvDefId:%s, name:%s, desc:%s, params:%s, params_type:%s)>" % (
            self.id, self.service_definition_id, self.name, self.desc, self.params, self.params_type)
