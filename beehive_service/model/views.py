...
class ServiceInstantConsume(AuditData, Base):
    ...
    def __init__(self, plugin_name, group_name, instant_value, unit, value, service_instance_id, account_id,
                 job_id, creation_date=None):
    ...
    job_id = Column('fk_job_id', Integer(), ForeignKey(
        'service_job.id'), nullable=False)

    def __repr__(self):
        return '<Model:ServiceInstanteConsume(id=%s, plugin_name=%s, service_instance_id=%s, account_id=%s, ' \
               'metric_group_name=%s, metric_instant_value=%s, metric_value=%s)>' % \
               (self.id, self.plugin_name, self.service_instance_id, self.account_id, self.metric_group_name,
                self.metric_instant_value, self.metric_value)
