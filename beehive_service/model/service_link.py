...
class ServiceLink(Base, BaseEntity):
    ...
    start_service = relationship(
        'ServiceInstance', foreign_keys=start_service_id)
    ...
    tag = relationship('ServiceTag', secondary=tags_links,
                       backref=backref('service_link', lazy='dynamic'))

    def __init__(self, objid, name, ltype, start_service, end_service,
                 attributes=''):
    ...
    def __repr__(self):
        return '<%s id=%s, uuid=%s, obid=%s, name=%s, type=%s, start=%s, end=%s>' % (
            self.__class__.__name__, self.id, self.uuid, self.objid, self.name, self.type, self.start_service_id,
            self.end_service_id)