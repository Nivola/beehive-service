...
from sqlalchemy import Table, Column, Integer, ForeignKey, String, UniqueConstraint, Text
...
account_account_capabilities = Table(
    ...
    Column('fk_account_capabilities', Integer(),
           ForeignKey('account_capabilities.id')),
    Column('status', String(20), default=SrvStatusType.BUILDING, nullable=False),
    UniqueConstraint('fk_account_id', 'fk_account_capabilities',
...
class AccountCapabilityAssoc(Base):
    ...
    def association_seq(self):
        return self.capability.name, self.capability.plugin_name, self.status,
...
class AccountCapability(ApiBusinessObject, Base):
    ...
    status = Column('status', String(20), ForeignKey('service_status.name'), default=SrvStatusType.ACTIVE,
                    nullable=False)
    ...
    def __init__(self, objid, name, desc, plugin_name='ComputeService', active=True, version='1.0',
                 status=SrvStatusType.ACTIVE, params='{}'):
    ...
    def __repr__(self):
        return "<Model:AccountCapability(id:%s, objid:%s, name:%s)>" % (self.id, self.objid, self.name)