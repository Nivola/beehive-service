...
class AggregateCost(AuditData, Base):
    ...
    def __init__(self, metric_type_id, consumed, cost, service_instance_id, account_id, aggregation_type, period,
                 cost_type_id, evaluation_date=datetime.today(), job_id=None):
    ...
    __table_args__ = (Index('udx_aggregate_cost_1', "fk_service_instance_id", "fk_account_id", "aggregation_type",
                            "period", "fk_metric_type_id", unique=True), {'mysql_engine': 'InnoDB'})
    ...
    service_instance_id = Column('fk_service_instance_id', Integer(
    ), ForeignKey('service_instance.id'), nullable=True)
    ...
    cost_type_id = Column('fk_cost_type_id', Integer(),
                          ForeignKey('cost_type.id'), nullable=False)
    cost_type = relationship('CostType')

    def __repr__(self):
        if 'daily' == self.aggregation_type:
            return "<Model:AggregateCost daily (id:%s, type=%s, consumed:%s, cost:%s, eval_date:%s, inst_id:%s, " \
                   "cost_type_id:%s)>" % (self.id, self.metric_type_id, self.consumed, self.cost, self.evaluation_date,
                                          self.service_instance_id, self.cost_type_id)
        else:
            return "<Model:AggregateCost monthly (id:%s, type=%s, consumed:%s, cost:%s, eval_date:%s, acc_id:%s, " \
                   "cost_type_id:%s)>" % (self.id, self.metric_type_id, self.consumed, self.cost, self.evaluation_date,
                                          self.account_id, self.cost_type_id)
