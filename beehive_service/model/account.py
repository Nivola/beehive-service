...
class Account(ApiBusinessObject, Base):
    ...
    service_instances = relationship(
        ...
        "ServiceInstance.expiry_date>=func.now()))",
    )  # ,lazy='dynamic')
    ...
    applied_bundles = relationship("AppliedBundle", foreign_keys="AppliedBundle.account_id")
...
class AccountServiceDefinition(ApiBusinessObject, Base):
    ...
    """
    account_id: int = Column("fk_account_id", Integer, ForeignKey("account.id"))
    account: Account = relationship("Account")
    service_definition_id: int = Column("fk_service_definition_id", Integer, ForeignKey("service_definition.id"))
