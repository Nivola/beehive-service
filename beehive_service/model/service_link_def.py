...
class ServiceLinkDef(ApiBusinessObject, Base):
    """ ServiceLinkDef
    """
    __tablename__ = 'service_link_def'
    __table_args__ = (
        Index('idx_srvlnkdef_1', 'start_service_id',
              'end_service_id', unique=True),
        {'mysql_engine': 'InnoDB'}
    )

    start_service_id = Column(Integer(), ForeignKey('service_definition.id'))
    start_service = relationship(
        'ServiceDefinition', foreign_keys=start_service_id, back_populates="linkChildren")
    end_service_id = Column(Integer(), ForeignKey('service_definition.id'))
    end_service = relationship(
        'ServiceDefinition', foreign_keys=end_service_id)
    ...
    def __repr__(self):
        return '<%s id=%s, uuid=%s, obid=%s, name=%s, start=%s, end=%s, priority=%s>' % (
            ...
            self.name, self.start_service_id,
