...
class Agreement(ApiBusinessObject, Base):
    ...
    wallet_id = Column("fk_wallet_id", Integer(), ForeignKey("wallet.id"), nullable=False)
    wallet = relationship("Wallet", back_populates="agreements")
    service_status_id = Column("fk_service_status_id", Integer, ForeignKey("service_status.id"))
...
class Wallet(ApiBusinessObject, Base):
    ...
    division_id = Column("fk_division_id", Integer(), ForeignKey("division.id"), nullable=False)
    ...
    agreements = relationship(
        ...
        "Agreement.expiry_date>=func.now()))",
        lazy="dynamic",
    )
    service_status_id = Column("fk_service_status_id", Integer, ForeignKey("service_status.id"))
    status = relationship("ServiceStatus")
...
class ReportCost(AuditData, Base):
    ...
    def __repr__(self):
        return (
            "<Model:ReportCost(account_id=%s, value=%s, cost=%s, type=%s, period=%s, report_date=%s, "
            ...
            % (
                ...
                self.report_date,
                self.plugin_name,
                self.metric_type_id is not None,
            )
        )
...
class ServiceCostParam(BaseApiBusinessObject, Base):
    ...
    def __repr__(self):
        return "<Model:ServiceCostParam(%s, %s, %s, %s)>" % (
            self.id,
            self.uuid,
            self.name,
            self.active,
...
class MetricTypePluginType(Base, AuditData):
    ...
    id = Column(Integer, primary_key=True)
    plugin_type_id = Column("fk_plugin_type_id", Integer(), ForeignKey("service_plugin_type.id"))
    metric_type_id = Column("fk_metric_type_id", Integer(), ForeignKey("service_metric_type.id"))
...
class DivisionsPrices(AuditData, Base):
    ...
    id = Column(Integer, primary_key=True)
    division_id = Column("fk_division_id", Integer, ForeignKey("division.id"), nullable=False)
    price_list_id = Column("fk_price_list_id", Integer, ForeignKey("service_pricelist.id"), nullable=False)
    start_date = Column(DateTime(), nullable=False)
    end_date = Column(DateTime(), nullable=True)
...
class ServicePriceMetricThresholds(Base):
    ...
    __table_args__ = (
        Index(
            "udx_service_price_metric_threshold",
            "from_ammount",
            "fk_service_price_metric_id",
            unique=True,
        ),
        {"mysql_engine": "InnoDB"},
    )
    ...
    service_price_metric_id = Column(
        "fk_service_price_metric_id",
        Integer(),
        ForeignKey("service_price_metric.id"),
        nullable=False,
    )
...
class ServicePriceMetric(BaseApiBusinessObject, Base):
    ...
    __table_args__ = (
        Index("udx_price_metric_1", "fk_price_list_id", "fk_metric_type_id", unique=True),
        {"mysql_engine": "InnoDB"},
    )
    ...
    metric_type_id = Column("fk_metric_type_id", Integer(), ForeignKey("service_metric_type.id"))
    price_list_id = Column("fk_price_list_id", Integer(), ForeignKey("service_pricelist.id"))
    price_list = relationship("ServicePriceList", back_populates="metrics_prices")
    thresholds = relationship("ServicePriceMetricThresholds", backref="fk_service_price_metric_id")
...
AggregateCostMonth = Table(
    ...
    Column("fk_metric_type_id", Integer(), ForeignKey("service_metric_type.id")),
