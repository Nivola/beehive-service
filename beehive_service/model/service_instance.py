...
tag_instance = Table(
    ...
    Column("fk_service_instance_id", Integer(), ForeignKey("service_instance.id")),
    mysql_engine="InnoDB",
)
...
class ServiceInstanceConfig(BaseApiBusinessObject, Base):
    ...
    def __repr__(self):
        return "<ServiceInstanceConfig(%s, %s, %s)>" % (
            self.id,
            self.name,
