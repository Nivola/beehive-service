...
class ServiceLinkInstance(ApiBusinessObject, Base):
    ...
    __table_args__ = (
        Index("idx_srvlnkinst_1", "start_service_id", "end_service_id", unique=True),
        {"mysql_engine": "InnoDB"},
    )
    start_service_id = Column(Integer(), ForeignKey("service_instance.id"))
    start_service = relationship("ServiceInstance", foreign_keys=start_service_id, back_populates="linkChildren")
