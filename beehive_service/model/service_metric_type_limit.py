...
class ServiceMetricTypeLimit(Base):
    ...
    __table_args__ = (Index('udx_srv_metric_type_limit_1', 'parent_id', 'fk_metric_type_id', unique=True),
                      Index('udx_srv_metric_type_limit_2',
    ...
    parent_id = Column('parent_id', Integer(), ForeignKey(
        'service_metric_type.id'), nullable=False)
    metric_type_id = Column('fk_metric_type_id', Integer(), ForeignKey(
        'service_metric_type.id'), nullable=False)

    def __repr__(self):
        return '<%s id=%s, name=%s, value=%s, parent_id=%s, metric_type_id=%s>' % (
            self.__class__.__name__, self.id, self.name, self.value, self.parent_id, self.metric_type_id)