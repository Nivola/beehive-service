...
class ServiceCatalog(ApiBusinessObject, Base):
    ...
    service_definitions = relationship("ServiceDefinition", secondary="catalog_definition",
    ...
    def __repr__(self):
        return "<Model:ServiceCatalog(id:%s, objid:%s, name:%s)>" % (self.id, self.objid, self.name)
...
catalog_definition = Table(
    ...
    Column('fk_service_catalog_id', Integer(),
           ForeignKey('service_catalog.id')),
    Column('fk_service_definition_id', Integer(),
           ForeignKey('service_definition.id')),
    UniqueConstraint('fk_service_catalog_id',
                     'fk_service_definition_id', name='idx_cat_def'),
    mysql_engine='InnoDB')