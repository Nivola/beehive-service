...
class ApiMonitoringFolder(AsyncApiServiceTypePlugin):
    ...
    def customize_list(controller, entities, *args, **kvargs):
        ...
        account_idx = controller.get_account_idx()
        compute_service_idx = controller.get_service_instance_idx(ApiMonitoringFolder.plugintype, index_key='account_id')
    ...
    def aws_info(self):
        ...
        instance_item['endpoints'] = {
            ...
            'home': '%s/dashboards/f/%s' % (base_endpoint, grafana_folder_ext_id)
    ...
    def sync_users(self):
        ...
        try:
            ...
            team_data = {
                'parent': resource_folder_id
            }
            res_team = self.controller.api_client.admin_request('resource', '/v1.0/nrs/provider/monitoring_teams', 'get',
                                                                data=team_data, other_headers=None)
    ...
    def list_resources(self, zones=None, uuids=None, tags=None, page=0, size=-1):
        ...
        folders = self.controller.api_client.admin_request('resource', uri, 'get', data=data).get('monitoring_folders', [])
    ...
    def create_resource(self, task, *args, **kvargs):
        ...
        if uuid is not None and taskid is not None:
            ...
            resource_team_id = self.__create_resource_team(task, container, team_viewer_name, resource_folder_id, norescreate)

            # create alert notification
            alert_name = self.__get_alert_name(triplet)
            self.__create_resource_alert(task, container, alert_name, resource_folder_id, account_email, norescreate)
            ...
            if norescreate is False:
                ...
                self.__create_resource_dashboard(task, resource_folder_id, organization, division, account, dashboard, triplet)
    ...
    def __create_resource_team(self, task, container, team_name, resource_folder_id, norescreate):
        ...
        # create team
        try:
            res = self.controller.api_client.admin_request('resource', '/v1.0/nrs/provider/monitoring_teams', 'post',
                                                           data=team_data, other_headers=None)
            taskid = res.get('taskid', None)
            uuid = res.get('uuid', None)
        except ApiManagerError as ex:
    ...
    def __create_resource_dashboard(self, task, resource_folder_id, organization, division, account, dashboard, triplet):
        """Create dashboard
        ...
        for dashboard_item in dashboard:
            ...
            dash_tag = None
            if 'k' in dashboard_item:
                dash_tag = dashboard_item['tag']

            dashboard_data = {
                'action': {
                    'add_dashboard': {
                        # 'folder_id_from': dashboard_folder_from,
    ...
    def __create_resource_permission(self, task, resource_folder_id, team_viewer_name):
        ...
        try:
            ...
            res = self.controller.api_client.admin_request('resource', url_action, 'put',
                                                            data=permission_data, other_headers=None)
            taskid = res.get('taskid', None)
            uuid = res.get('uuid', None)
        except ApiManagerError as ex:
            self.logger.error(ex, exc_info=True)
    ...
    def __create_resource_users(self, task, resource_team_id, str_users):
        ...
        try:
            ...
            res = self.controller.api_client.admin_request('resource', url_action, 'put', data=user_data, other_headers=None)
            taskid = res.get('taskid', None)
            uuid = res.get('uuid', None)
        except ApiManagerError as ex:
            self.logger.error(ex, exc_info=True)
    ...
    def __create_resource_alert(self, task, container, alert_name, resource_folder_id, account_email, norescreate):
        """Create alert notification
    ...
    def update_resource(self, task, **kvargs):
        ...
        try:
            ...
            # base update
                ...
                data = {
                    'monitoring_folder': kvargs
                }
                self.controller.api_client
                api_client: ApiClient = self.controller.api_client
                res = api_client.admin_request(
                    'resource', '/v1.0/nrs/provider/monitoring_folders/%s' % self.instance.resource_uuid, 'put', data=data)
    ...
    def __delete_team(self, task):
        ...
        if len(monitoring_teams) > 0:
            ...
            for monitoring_team in monitoring_teams:
                ...
                res_team_delete = self.controller.api_client.admin_request('resource', uri, 'delete',
                                                                        data=team_delete_data, other_headers=None)
    ...
    def __delete_alert(self, task):
        ...
        uri = '/v1.0/nrs/provider/monitoring_alerts'
        res_alert = self.controller.api_client.admin_request('resource', uri, 'get', data=alert_data, other_headers=None)
        ...
        if len(monitoring_alerts) > 0:
            ...
            res_alert_delete = self.controller.api_client.admin_request('resource', uri, 'delete', data=alert_delete_data, other_headers=None)
    ...
    def action_resource(self, task, *args, **kvargs):
        ...
        if action is not None:
            ...
            if name == 'sync-users':
                ...
                resource_team_id = args.get('resource_team_id')
...
class ApiMonitoringInstance(AsyncApiServiceTypePlugin):
    ...
    def create_resource(self, task, *args, **kvargs):
        ...
        templates = None
        data = {
            'action': {'enable_monitoring': {'templates': templates}}
        }
        ...
        compute_service_instance: ApiServiceInstance = self.controller.get_service_instance(compute_instance_id)
        self.add_link(name='monitoring-%s' % id_gen(), type='monitoring', end_service=compute_service_instance.oid,
                      attributes={})
