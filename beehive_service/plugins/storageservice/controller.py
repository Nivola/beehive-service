...
from beehive_service.service_util import __SRV_STORAGE_PROTOCOL_TYPE_NFS__, __SRV_STORAGE_PROTOCOL_TYPE_CIFS__
...
class ApiStorageService(ApiServiceTypeContainer):
    ...
    @staticmethod
    def customize_list(controller: ServiceController, entities: List['ApiStorageService'], *args, **kvargs):
...
class ApiStorageEFS(AsyncApiServiceTypePlugin):
    ...
    def pre_delete(self, **params):
        ...
        if self.check_resource() is not None:
            raise ApiManagerError('File system %s has an active mount target. It can not be deleted' %
                                  self.instance.uuid)
        return params
    ...
    def pre_action(self, **params):
        ...
        if action == 'create_mount_target':
            ...
            params['steps'] = [self.task_path + 'delete_mount_grants_step',
    ...
    def create_mount_target_resource(self, task, **data):
        ...
        res = self.controller.api_client.admin_request('resource', '/v1.0/nrs/provider/shares', 'post',
                                                       data={'share': data})
        ...
        if len(grants) > 0:
            ...
            for grant in grants:
                ...
                if job is not None:
                    ...
                    self.logger.debug('Deleted grant resource %s in file system instance %s res=%s' %
                                      (data, self.instance.uuid, res))
        else:
    ...
    def update_resource(self, task, size=None, **kwargs):
        ...
        res = self.controller.api_client.admin_request(
            'resource',  '/v1.0/nrs/provider/shares/%s' % self.instance.resource_uuid, 'put', data=data)
        job= res.get('taskid')
        if job is not None:
            self.wait_for_task(job, delta=2, maxtime=180, task=task)
        return job

    def do_file_system_grant_op(self, task, action='add', access_level='ro', access_type='ip', access_to=None,
                                access_id=None):
        ...
        if action == 'add':
            ...
            raise ApiManagerError(u"Unknown grant action :%s" % action)
        ...
        if job is not None:
            self.wait_for_task(job, delta=2, maxtime=180, task=task)
        self.logger.debug('Created grant resource %s in file system instance %s res=%s' %
                          (data, self.instance.uuid, res))
        return job
