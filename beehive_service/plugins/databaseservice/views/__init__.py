...
class DescribeDatabaseServiceResponseSchema(Schema):
    DescribeDatabaseResponse = fields.Nested(DescribeDatabaseResponseSchema, required=True, many=False,
                                                allow_none=False)
...
class DescribeAccountAttributes(ServiceApiView):
    ...
    def get(self, controller, data, *args, **kvargs):
        ...
        if tot > 0:
            ...
            raise ApiManagerError('Account %s has no database instance associated' % data.get('owner_id'),code=404)
...
class DeleteDatabaseService(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        202: {
            'description': 'success',
            'schema': DeleteDatabaseServiceResponseSchema
        }
    })
...
class DatabaseServiceAPI(ApiView):
    ...
    def register_api(module, rules=None, **kwargs):
        ...
        rules = [
            ...
            ('%s/databaseservices/describeaccountattributes' % base, 'GET', DescribeAccountAttributes, {}),
            ('%s/databaseservices/modifyaccountattributes' % base, 'PUT', ModifyAccountAttributes, {})
        ]
