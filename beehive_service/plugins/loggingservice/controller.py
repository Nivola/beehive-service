...
class ApiLoggingSpace(AsyncApiServiceTypePlugin):
    ...
    def create_resource(self, task, *args, **kvargs):
        ...
        if uuid is not None and taskid is not None:
            ...
            #                                  str_users)
            self.__create_resource_role_mapping(
                task,
                container,
                triplet,
                resource_space_id,
    ...
    def __create_resource_role(self, task, container, triplet, resource_space_id, space_id, norescreate):
        ...
        try:
            res = self.controller.api_client.admin_request(
                "resource",
                "/v1.0/nrs/provider/logging_roles",
                "post",
                data=role_data,
                other_headers=None,
            )
    ...
    def __create_resource_role_mapping(
        ...
        try:
            ...
            res = self.controller.api_client.admin_request(
                "resource", uri, "post", data=role_mapping_data, other_headers=None
            )
            taskid = res.get("taskid", None)
            uuid = res.get("uuid", None)
        except ApiManagerError as ex:
    ...
    def __delete_role(self, task):
        ...
        role_data = {"parent": self.instance.resource_uuid}
        res_role = self.controller.api_client.admin_request(
            "resource",
            "/v1.0/nrs/provider/logging_roles",
            "get",
            data=role_data,
        ...
        if len(logging_roles) > 0:
            ...
            res_role_delete = self.controller.api_client.admin_request(
                "resource", uri, "delete", data=role_delete_data, other_headers=None
            )
    ...
    def __delete_role_mapping(self, task):
        ...
        role_mapping_data = {"parent": self.instance.resource_uuid}
        uri = "/v1.0/nrs/provider/logging_role_mappings"
        res_role_mapping = self.controller.api_client.admin_request(
            "resource", uri, "get", data=role_mapping_data, other_headers=None
        )
        ...
        if len(logging_role_mappings) > 0:
            ...
            res_role_mapping_delete = self.controller.api_client.admin_request(
                "resource",
                uri,
                "delete",
    ...
    def action_resource(self, task, *args, **kvargs):
        ...
        if action is not None:
            ...
            if name == "sync-users":
                ...
                resource_space_id = action_args.get("resource_space_id")
                role_name = action_args.get("role_name")
