...
class LoggingSetResponseSchema(Schema):
    ...
    stateReason = fields.Nested(LoggingStateReasonResponseSchema, many=False, required=True, description='state description')
...
class DescribeLoggingResponseInnerSchema(Schema):
    ...
    loggingTotal = fields.Integer(required=False, example='0', descriptiom='total logging', data_key='loggingTotal')
...
class DescribeAccountAttributes(ServiceApiView):
    ...
    def get(self, controller, data, *args, **kvargs):
        ...
        res, tot = controller.get_service_type_plugins(account_id_list=[data.get('owner_id')],
                                                       plugintype=ApiLoggingService.plugintype)
        if tot > 0:
            apiLoggingService: ApiLoggingService = res[0]
            attribute_set = apiLoggingService.aws_get_attributes()
        else:
...
#     summary = 'Modify attributes of logging service'
#     description = 'Modify attributes of logging service'
#     tags = ['loggingservice']
#     definitions = {
...
class DeleteLoggingService(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': DeleteLoggingServiceResponseSchema
        }
    })
...
class LoggingServiceAPI(ApiView):
    ...
    def register_api(module, rules=None, **kwargs):
        ...
        rules = [
            ...
            ('%s/loggingservices/describeaccountattributes' % base, 'GET', DescribeAccountAttributes, {}),
