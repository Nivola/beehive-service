...
from beehive_service.plugins.computeservice.controller import ApiComputeSubnet, ApiComputeSecurityGroup, ApiComputeVPC, \
...
class ApiAppEngineServiceResourceHelper(object):
    ...
    def list_instance_resources(self, zones=None, uuids=None, page=0, size=20):
        ...
        res = self.controller.api_client.admin_request('resource', '/v1.0/nrs/provider/app_stacks', 'get',
                                                       data=urlencode(data)).get('app_stacks', [])
...
class ApiAppEngineService(ApiServiceTypeContainer):
    ...
    def import_instance(controller, resource_id, inst_def):
        ...
        res, total = controller.get_paginated_service_instances(resource_uuid=compute_zone,
        ...
        controller.check_authorization(ApiServiceInstance.objtype, ApiServiceInstance.objdef, account.objid, 'insert')
        ...
        if inst_compute_service.verify_permisssions('update') is False:
            raise ApiManagerWarning(
                'User does not have permission to update instance id %s' % inst_compute_service.oid)

        # create service
        instance = controller.createInstanceHierachy(None, None, name=name, desc=desc,
                                                     instance_parent_id=inst_compute_service.oid,
                                                     service_def=inst_def, account=account)
        plugin_root = ApiServiceInstance(controller, oid=instance.id, objid=instance.objid, name=instance.name,
                                         desc=instance.desc, active=instance.active, model=instance)

        # get vpc
        vpcs, total = controller.get_paginated_service_instances(resource_uuid=vpc_resource_uuid,
        ...
        # get security group
        sgs, total = controller.get_paginated_service_instances(resource_uuid=vpc_resource_uuid,
        ...
        for inst_cfg in inst_cfgs:
            ...
            json_cfg_merged = merge_dicts(inst_cfg.json_cfg, {'computeZone': inst_compute_service.resource_uuid},
                                          data)
...
class ApiAppEngineInstance(AsyncApiServiceTypePlugin):
    ...
    def pre_create(self, **params):
        ...
        if is_public is True:
            ...
            inst_public_subnet = self.controller.check_service_instance(public_subnet_id, ApiComputeSubnet,
                                                                        account=account_id)
        ...
        for securityGroup in securitygroup_ids:
            sg_inst = self.controller.check_service_instance(securityGroup, ApiComputeSecurityGroup,
                                                             account=account_id)
    ...
    def list_resources(self, zones=None, uuids=None, tags=None, page=0, size=100):
        ...
        instances = self.controller.api_client.admin_request('resource', '/v1.0/nrs/provider/app_stacks', 'get',
                                                             data=urlencode(data)).get('app_stacks', [])
