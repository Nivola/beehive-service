...
from beehive_service.plugins.appengineservice.controller import ApiAppEngineService, ApiAppEngineInstance, \
...
from beehive_service.plugins.computeservice.controller import ApiComputeSubnet,\
    ApiComputeSecurityGroup, ApiComputeVPC, ApiComputeService
...
class InstanceConfigsRequestSchema(Schema):
    documentRoot = fields.String(required=False, example='/var/www',
                                 description='[apache-php] document root')
    ftpServer = fields.Boolean(required=False, example=True, default=True,
                               description='[apache-php] if true install ftp server')
    smbServer = fields.Boolean(required=False, example=False, default=False,
                               description='[apache-php] if true install samba server')
    shareDimension = fields.Integer(required=False, example=10, default=10,
                                    description='[apache-php] share dimension in GB')
    shareCfgDimension = fields.Boolean(required=False, example=2, default=2,
                                       description='[apache-php] share config dimension in GB')
    appPort = fields.Integer(required=False, example=80, default=80,
                             description='[apache-php] internal application prot')
    farmName = fields.String(required=True, example='tst-portali',
...
class InstanceAppEngineSetResponseSchema(Schema):
    ...
    privateIpAddresses = fields.List(fields.String(example='192.168.1.79'), required=True,
                                     description='List of internal server ip adresses')
    uris = fields.List(fields.String(example='https://192.168.1.98:443'), description='list of app engine uris',
                       required=True)
    ...
    engineConfigs = fields.Nested(InstanceConfigsRequestSchema, required=True,
...
class DescribeAppInstancesResponseSchema(Schema):
    DescribeAppInstancesResponse = fields.Nested(DescribeAppInstancesApi1ResponseSchema, required=True, many=False,
                                              allow_none=False)
...
class DescribeAppInstances(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': DescribeAppInstancesResponseSchema
        }
    })
...
class CreateAppInstancesApiParamConfigsRequestSchema(Schema):
    ...
    SmbServer = fields.Boolean(required=False, example=False, default=False,
                               description='[apache-php] if true install samba server')
    ShareDimension = fields.Integer(required=False, example=10, missing=10,
                                    description='[apache-php] share dimension in GB')
    ShareCfgDimension = fields.Boolean(required=False, example=2, default=2,
                                       description='[apache-php] share config dimension in GB')
    AppPort = fields.Integer(required=False, example=80, default=80,
...
class CreateAppInstancesApiParamRequestSchema(Schema):
    ...
    PublicSubnetId = fields.String(required=False, example='12',
                                   description='instance id or uuid of the public subnet')
    IsPublic = fields.Boolean(required=False, example=False, missing=False,
    ...
    KeyName = fields.String(required=False, example='1ffd', description='The name of the key pair')
    owner_id = fields.String(required=False, example='1', description='account id or uuid associated to compute zone')
    EngineConfigs = fields.Nested(CreateAppInstancesApiParamConfigsRequestSchema, required=False,
                                  description='App engine specific params')
...
class CreateAppInstances(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        202: {
            'description': 'success',
            'schema': CreateAppInstancesResponseSchema
        }
    })
...
class DeleteAppInstancesResponseSchema(Schema):
    DeleteAppInstancesResponse = fields.Nested(DeleteAppInstancesResponseItemSchema, required=True, many=False,
                                               allow_none=False)


class DeleteAppInstancesRequestSchema(Schema):
    InstanceId_N = fields.List(fields.String(example=''), required=True, allow_none=True,
                               context='query', collection_format='multi', data_key='InstanceId.N',
...
class DeleteAppInstances(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': DeleteAppInstancesResponseSchema
        }
    })
...
class AppEngineInstanceAPI(ApiView):
    ...
    def register_api(module, rules=None, **kwargs):
        base = 'nws'
        rules = [
            ('%s/appengineservices/instance/describeappinstances' % base, 'GET', DescribeAppInstances, {}),
            ('%s/appengineservices/instance/createappinstances' % base, 'POST', CreateAppInstances, {}),
            ('%s/appengineservices/instance/deleteappinstances' % base, 'DELETE', DeleteAppInstances, {}),
        ]
