...
class ApiComputeKeyPairsHelper(object):
    ...
    def get_private_rsa_fingerprint(self, key_location=None, key_file_obj=None, passphrase=None):
        ...
        RSA private key.
        """
        k = self.get_rsa_key(key_location=key_location, key_file_obj=key_file_obj,
...
# class ApiComputeServiceResourceHelper(object):
#     def __init__(self, controller):
#         self.controller = controller
#
...
#             res[r['uuid']] = r
#             res[r['name']] = r
#         self.controller.logger.debug('Index resources : %s' % truncate(str(res)))
#         return res
#
#     def index_resources_status(self, resources):
#         """Index resources status by uuid
#
#         :param resources: resource list
#         :return: resource status indexed by uuid
#         """
#         return {r['uuid']: r for r in resources.status}
#
#     def list_compute_service_resources(self, zones=None, uuids=None, page=0, size=-1):
#         """List compute service resource
#
...
#             data['uuids'] = uuids
#
#         res = self.controller.api_client.admin_request('resource', '/v1.0/nrs/provider/compute_zones', 'get',
#                                                        data=urlencode(data)).get('vpcs', [])
#         self.controller.logger.debug(
#             'Get compute service resources: %s' %
#             truncate(res))
#         return res
#
#     def get_compute_service_quotas(self, uuid):
#         """Get compute service quotas [DEPRECATED]
#
...
#         :raises ApiManagerError: raise :class:`.ApiManagerError`
#         """
#         uri = '/v1.0/nrs/provider/compute_zones/%s/quotas' % uuid
#         res = self.controller.api_client.admin_request(
#             'resource', uri, 'get', data='').get(
#             'quotas', [])
#         self.controller.logger.debug(
#             'Get compute service %s quotas: %s' %
#             (uuid, truncate(res)))
#         # quotas = {}
#         # for item in res:
#         #     key = item.pop('quota')
...
#         return res
#
#     def update_compute_service_quotas(self, uuid, quotas):
#         """Update compute service quotas
#
#         :param uuid: resource uuid
...
#         }
#         res = self.controller.api_client.admin_request(
#             'resource', uri, 'put', data=data)
#         self.controller.logger.debug(
#             'Update compute service %s quotas: %s' %
#             (uuid, truncate(res)))
#         return res
...
class ApiComputeService(ApiServiceTypeContainer):
    ...
    def __init__(self, *args, **kvargs):
        ...
        # self.api_helper = ApiComputeServiceResourceHelper(self.controller)
    ...
    @staticmethod
    def customize_list(controller: ServiceController, entities: List[ApiComputeService], *args, **kvargs) \
    ...
    def change_definition(self, inst_service, def_id):
        ...
        self.logger.debug(
            'Change compute service %s definition to %s' %
            (inst_service.uuid, def_id))
        return True
    ...
    def get_compute_instance_backup(self):
        ...
        instance = ApiComputeInstanceBackup(self.controller, oid=self.oid, objid=self.objid, name=self.name,
                                            desc=self.desc, active=self.active, model=self.model, compute_service=self)
        return instance
...
class ApiComputeInstanceBackup(AsyncApiServiceTypePlugin):
    ...
    def verify_permisssions(self, action='use'):
        self.controller.check_authorization(self.compute_service.instance.objtype,
                                            self.compute_service.instance.objdef,
                                            self.compute_service.instance.objid, action)
    ...
    def list_jobs(self, job_id=None):
        ...
        if job_id is None:
            jobs = self.list_resource_jobs()
        else:
            jobs = [self.get_resource_job(job_id)]

        res = [self.aws_job_info(i) for i in jobs]
        self.logger.debug('get compute service %s backup jobs: %s' % (self.compute_service.instance.uuid, truncate(res)))
    ...
    def add_job(self, account, name, desc, site, flavor, instance_ids, hypervisor='openstack'):
        ...
        for instance_id in instance_ids:
            ...
            if plugin.get_availability_zone() != site:
                raise ApiManagerError('compute instance %s and backup job availability zone does not match' %
                                      plugin.instance.uuid)
            if plugin.get_hypervisor() != hypervisor:
                raise ApiManagerError('compute instance %s and backup job hypervisor does not match' %
                                      plugin.instance.uuid)
        ...
        # add job
        res = self.add_resource_job(name, desc, site, fullbackup_interval, restore_points, start_time, instance_resource_ids,
                                    hypervisor=hypervisor, hypervisor_tag=hypervisor_tag, timezone=timezone,
                                    interval=interval, job_type='Parallel')
    ...
    def update_job(self, job_id, name=None, flavor=None, enabled=None):
        ...
        # add job
        res = self.update_resource_job(job_id, name=name, fullbackup_interval=fullbackup_interval,
                                       restore_points=restore_points, enabled=enabled)
        self.logger.debug('update compute service %s backup job: %s' % (self.compute_service.instance.uuid,
                                                                        truncate(res)))
        return res
    ...
    def add_instance_to_job(self, job_id, instance_id):
        ...
        self.logger.debug('add compute instance %s to compute service %s backup job %s' %
    ...
    def del_instance_from_job(self, job_id, instance_id):
        ...
        # chek job exists
        job = self.exist_backup_job(job_id)
        job_instance_uuids = [self.controller.get_service_instance_by_resource_uuid(i.get('uuid')).uuid
                              for i in job.get('instances', [])]
    ...
    def exist_backup_job(self, job_id):
        try:
            ...
            raise ApiManagerError('compute service %s backup job %s does not exist' %
                                  (self.compute_service.instance.uuid, job_id))

        return job

    def exist_backup_job_restore_point(self, job_id, restore_point_id):
        restore_points = self.get_resource_backup_job_restore_points(self, job_id, restore_point_id=restore_point_id)

        if len(restore_points) < 1:
            err = 'compute service %s backup job %s restore point %s does not exist' % \
                  (self.compute_service.instance.uuid, job_id, restore_point_id)
            self.logger.error(err)
            raise ApiManagerError(err)

        return restore_points[0]

    def get_job_restore_points(self, job_id, restore_point_id=None):
        ...
        res = self.get_resource_backup_job_restore_points(job_id, restore_point_id=restore_point_id)
        res = [self.aws_restore_point_info(item) for item in res]
        self.logger.debug('get compute instance %s backup resource points: %s' % (self.compute_service.instance.uuid,
                                                                                  truncate(res)))
        return res
    ...
    def add_job_restore_point(self, job_id, name, desc=None, full=True):
        ...
        taskid = self.add_job_restore_point(self, job_id, name, desc=desc, full=full)
        return taskid
    ...
    def del_job_restore_point(self, job_id, restore_point_id):
        ...
        # create restore point
        taskid = self.del_job_restore_point(job_id, restore_point_id)
        return taskid

    #
    # resource client method
    #
    def list_resource_jobs(self):
        ...
        instances = self.controller.api_client.admin_request('resource', uri, 'get', data='').get('jobs', [])
    ...
    def get_resource_job(self, job_id):
        ...
        :raises ApiManagerError: raise :class:`.ApiManagerError`
        """
        uri = '/v1.0/nrs/provider/compute_zones/%s/backup/jobs/%s' % (self._resource_uuid, job_id)
    ...
    def add_resource_job(self, name, desc, site, fullbackup_interval, restore_points, start_time, instances,
                         hypervisor='openstack', hypervisor_tag='default', timezone='Europe/Rome', interval='24hrs',
                         job_type='Parallel'):
    ...
    def update_resource_job(self, job_id, name=None, fullbackup_interval=None, restore_points=None, enabled=None):
    ...
    def get_resource_backup_job_restore_points(self, job_id, restore_point_id=None):
        ...
        backup = self.controller.api_client.admin_request('resource', uri, 'get', data=data).get('restore_points', [])
        self.logger.debug('Get compute instance resource backup restore points: %s' % truncate(backup))
        return backup
...
class ApiComputeImage(AsyncApiServiceTypePlugin):
    ...
    @staticmethod
    def customize_list(controller: ServiceController, entities: List[ApiComputeImage], *args, **kvargs) \
    ...
    def list_resources(self, zones=[], uuids=[], tags=[], page=0, size=-1):
        ...
        images = self.controller.api_client.admin_request('resource', '/v1.0/nrs/provider/images', 'get',
                                                          data=urlencode(data)).get('images', [])
...
class ApiComputeInstance(AsyncApiServiceTypePlugin):
    ...
    @staticmethod
    def customize_list(controller: ServiceController, entities: List[ApiComputeInstance], *args, **kvargs)\
        ...
        image_idx = controller.get_service_instance_idx(ApiComputeImage.plugintype)
        security_group_idx = controller.get_service_instance_idx(ApiComputeSecurityGroup.plugintype,
                                                                 index_key='resource_uuid')
        ### TODO aggiungi filtri per evitare di caricare tutte le istanze per plugin
        volume_idx = controller.get_service_instance_idx(ApiComputeVolume.plugintype, index_key='resource_uuid')
        compute_service_idx = controller.get_service_instance_idx(ApiComputeService.plugintype, index_key='account_id')
    ...
    def get_volume_type_by_resource(self, resource):
        ...
        """
        res = None
        servdefs, tot = self.controller.get_paginated_service_defs(plugintype=ApiComputeVolume.plugintype,
    ...
    def get_instance_type_name(self):
        flavor_label = self.get_host_group()
        flavor_resource_name = dict_get(self.resource, 'flavor.name')
        if self.get_hypervisor() == 'openstack' and flavor_label is not None:
            flavor_resource_name = flavor_resource_name.replace('.'+flavor_label, '')

        self.logger.warn(flavor_label)
        self.logger.warn(flavor_resource_name)
    ...
    def aws_info(self, version='v1.0'):
        ...
        if self.get_config('instance') is not None:
            ...
            key_name = dict_get(self.resource, 'attributes.key_name')
            # key_name = config.get('KeyName')

            # get subnet
            # subnet = self.controller.get_entity(ApiServiceInstance, ServiceInstance, subnet_id)
            subnet = self.subnet_idx.get(subnet_id)
            if subnet is not None:
                subnet_vpc = self.vpc_idx.get(subnet.get_parent_id())
                avzone = dict_get(self.resource, 'availability_zone.name')
        ...
        instance_item['instanceType'] = self.get_instance_type_name()
        # self.instance_type_idx.get(str(self.instance.service_definition_id)).name
        ...
        vpcs = self.resource.get('vpcs', None)
        # get only the first vpc
        if vpcs is not None and len(vpcs) > 0:
            vpc = vpcs[0]
        else:
            vpc = {}
        instance_item['privateDnsName'] = vpc.get('fixed_ip', {}).get('hostname', None)
        instance_item['dnsName'] = vpc.get('fixed_ip', {}).get('hostname', None)
        instance_item['privateIpAddress'] = vpc.get('fixed_ip', {}).get('ip', None)
        ...
        instance_item['hypervisor'] = self.resource.get('attributes', {}).get('type', None)
        instance_item['keyName'] = key_name
        instance_item['architecture'] = 'x86_64'
        instance_item['virtualizationType'] = 'full'
        instance_item['tagSet'] = [{'key': tag, 'value': None} for tag in self.tags]
        instance_item['networkInterfaceSet'] = []

        # storage
        blocks = []
        for item in self.resource.get('block_device_mapping', []):
            resource_vol_uuid = item.get('id', None)
            volume = self.volume_idx.get(resource_vol_uuid, None)
            if volume is not None:
                volume_id = volume.uuid
            else:
                volume_id = None
            blocks.append({
                'deviceName': item.get('path', None),
                'ebs': {
                    'volumeId': volume_id,
                    'status': 'attached',
                    'attachTime': item.get('attachment', None),
                    'deleteOnTermination': True,
                    'volumeSize': item.get('volume_size', None)
                }
            })
        ...
        flavor = self.resource.get('flavor', None)
        if flavor is not None:
            flavor.pop('uuid', None)
            flavor.pop('name', None)
    ...
    def pre_create(self, **params):
        ...
        # check the image id
        image_inst = self.controller.check_service_instance(data_instance.get('ImageId'), ApiComputeImage,
                                                            account=account_id)
        ...
        if image_configs.get('os') == 'Windows':
            ...
            insts, tot = self.controller.get_paginated_service_instances(name=hostname, filter_expired=False,
                                                                         authorize=False)
            if tot > 1:
        ...
        # set hostname for non windows server
            ...
            hostname = '%s%s' % (self.instance.name, self.controller.get_account_acronym(account_id))

            # check name is less than 30 characters
            if len(hostname) > 30:
                maxsize = 30 - len(self.controller.get_account_acronym(account_id))
                raise ApiManagerError('Name is too long. Linux ComputeInstance name max size is %s characters' %
                                      maxsize)
        ...
        if type_provider not in image_avz_hypervisors.get(av_zone):
            raise ApiManagerError('Image %s does not support hypervisor %s' %
                                  (data_instance.get('ImageId'), type_provider))
    ...
    def pre_import(self, **params):
        ...
        # get image
        images, tot = self.controller.get_paginated_service_instances(account_id=account_id, filter_expired=False,
                                                                      plugintype='ComputeImage', with_perm_tag=False,
                                                                      size=-1)
        image = None
        for image in images:
    ...
    def post_delete(self, **params):
        ...
        if resource is not None:
            for item in resource.get('block_device_mapping', []):
                resource_vol_uuid = item.get('id', None)
                volume, tot = self.controller.get_service_type_plugins(resource_uuid=resource_vol_uuid,
    ...
    def get_console(self):
        ...
        if self.get_hypervisor() not in ['openstack', 'vsphere']:
            raise ApiManagerError('native console is available only for compute instance running on '
    ...
    def get_job_restore_points(self):
        """Get backup job restore points
        ...
        res = self.get_resource_backup_job_restore_points()
        res = [self.aws_restore_point_info(item) for item in res]
        self.logger.debug('get compute instance %s backup resource points: %s' % (self.instance.uuid, truncate(res)))
        return res
    ...
    def check_restore_point(self, restore_point, accepted_status=None):
        ...
        rps = [s for s in restore_points
               if s['id'] == restore_point and s['status'] in accepted_status]
        if len(rps) > 0:
            return rps[0]
        return None
    ...
    def restore_from_backup(self, restore_point, instance_name):
        ...
        data.update({
            ...
            'Name': instance_name,
            'SecurityGroupId_N': [self.controller.get_service_instance_by_resource_uuid(s['uuid']).uuid
        ...
        # check account
        account, parent_plugin = self.controller.check_service_type_plugin_parent_service(
            account_id, ApiComputeService.plugintype)
        ...
        name = instance_name
        desc = instance_name
        inst = self.controller.add_service_type_plugin(service_definition_id, account_id, name=name, desc=desc,
                                                       parent_plugin=parent_plugin, instance_config=data)
        self.logger.info('restore instance %s from backup restore point %s on instance: %s' %
    ...
    def list_resources(self, zones=None, uuids=None, tags=None, page=0, size=-1):
        ...
        instances = self.controller.api_client.admin_request('resource', '/v1.0/nrs/provider/instances', 'get',
                                                             data=urlencode(data), timeout=600).get('instances', [])
    ...
    @trace(op='view')
    def get_resource_backup_job_restore_points(self):
        """Get resource backup
        ...
        if self.instance.resource_uuid is not None:
            ...
            backup = self.controller.api_client.admin_request('resource', uri, 'get', data='').get('restore_points', [])
        self.logger.debug('Get compute instance resource backup restore points: %s' % truncate(backup))
        return backup
    ...
    def get_resource_backup_restores(self, restore_point):
        ...
        if self.instance.resource_uuid is not None:
            uri = '/v1.0/nrs/provider/instances/%s/backup/restore_points/%s/restores' % \
                  (self.instance.resource_uuid, restore_point)
    ...
    def create_resource(self, task, *args, **kvargs):
        ...
        if restore_from_backup is True:
            ...
            try:
                ...
                data = {
                    'action': {
                        'restore_from_backup': {'restore_point': restore_point, 'instance_name': instance_name}
                    }
                }
    ...
    def delete_resource(self, task, *args, **kvargs):
        ...
        monitoring_attr = dict_get(resource, 'attributes.monitoring_enabled')
        
        if monitoring_attr is True:
            # self.logger.info('+++++ self.instance.status: %s' % self.instance.status)
            # self.logger.info('+++++ self.get_simple_runstate(): %s' % self.get_simple_runstate())
            if self.get_simple_runstate() == 'poweredOff':
                raise ApiManagerError('compute instance %s with monitoring enabled and stopped: cannot delete' % self.instance.uuid)
...
class ApiComputeKeyPairs(ApiServiceTypePlugin):
    ...
    @staticmethod
    def customize_list(controller: ServiceController, entities: List[ApiComputeKeyPairs], *args, **kvargs) \
    ...
    def __add_resource(self, **params):
        ...
        try:
            ...
            key_fingerprint = ApiComputeKeyPairsHelper(self).\
                get_private_rsa_fingerprint(key_file_obj=file_obj.getvalue())
            ...
            if account.managed is False:
                # useless set keyMaterial in config and delete it in aws_create_info
                # self.set_config('keyMaterial', b64_priv_key) # anche account privati sono managed!
                self.keyMaterial = b64_priv_key
            else:
                # self.set_config('keyMaterial', '') # set anche qui
                self.keyMaterial = b64_priv_key
...
class ApiComputeSecurityGroup(AsyncApiServiceTypePlugin):
    ...
    @staticmethod
    def customize_list(controller: ServiceController, entities: List[ApiComputeSecurityGroup], *args, **kvargs):
        """Post list function. Extend this function to execute some operation after entity was created. Used only for
        synchronous creation.
...
class ApiComputeSubnet(AsyncApiServiceTypePlugin):
    ...
    @staticmethod
    def customize_list(controller: ServiceController, entities: List[ApiComputeSubnet], *args, **kvargs) \
        ...
        compute_service_idx = controller.get_service_instance_idx(
            ApiComputeService.plugintype, index_key='account_id')
        vpc_idx = controller.get_service_instance_idx(ApiComputeVPC.plugintype)
...
class ApiComputeTag(ApiServiceTypePlugin):
    ...
    @staticmethod
    def customize_list(controller: ServiceController, entities: List[ApiComputeTag], *args, **kvargs) \
            -> List[ApiComputeTag]:
        """Post list function. Extend this function to execute some operation after entity was created. Used only for
        synchronous creation.
        ...
        for tag in entities:
            ...
            item.instance_type = import_class(
                tag.model.instance_objclass).objname
            tags.append(item)
        return tags
...
class ApiComputeVolume(AsyncApiServiceTypePlugin):
    ...
    @staticmethod
    def customize_list(controller: ServiceController, entities: List['ApiComputeVolume'], *args, **kvargs)\
    ...
    def attach(self, instance_id, device):
        ...
        if instance.get_hypervisor() != self.get_config('type'):
            raise ApiManagerError('volume %s and instance %s hypervisors mismatch' %
        ...
        if self.get_availability_zone() != instance.get_availability_zone():
            raise ApiManagerError('volume %s and instance %s availability zones are different' %
                                  (self.instance.uuid, instance.instance.uuid))

        params = {
            'resource_params': {
                'instance_id': instance.resource_uuid,
                'action': {'name': 'add_volume', 'args': {'volume': self.instance.resource_uuid}}
            }
        }
        res = self.update(**params)
    ...
    def detach(self, instance_id, device):
        ...
        params = {
            'resource_params': {
                'instance_id': instance.resource_uuid,
                'action': {'name': 'del_volume', 'args': {'volume': self.instance.resource_uuid}}
            }
        }
        res = self.update(**params)
    ...
    @trace(op='view')
    def list_resources(self, zones: List[str]=[], uuids: List[str]=[], tags: List[str]=[], page: int=0, size: int=-1) \
            -> dict :
        """Get resources info
        ...
        instances = self.controller.api_client.admin_request('resource', '/v1.0/nrs/provider/volumes', 'get',
                                                             data=urlencode(data)).get('volumes', [])
...
class ApiComputeVPC(AsyncApiServiceTypePlugin):
    ...
    @staticmethod
    def customize_list(controller: ServiceController, entities: List[ApiComputeVPC], *args, **kvargs) \
            -> List[ApiComputeVPC]:
        """Post list function. Extend this function to execute some operation after entity was created. Used only for
        synchronous creation.
...
class ApiComputeCustomization(AsyncApiServiceTypePlugin):
    ...
    def pre_create(self, **params):
        ...
        for instance in instances:
            ...
            self.instance.add_link(name='link-%s-%s' % (self.instance.oid, serv_inst.oid), type='instance',
                                   end_service=serv_inst.oid, attributes={})
        ...
        for name, value in customize_args.items():
            ...
            if arg_type is not None:
                if arg_type == 'int':
                    if not is_int(value):
                        raise Exception('Type of parameter %s is %s, should be %s' % (name, type(value).__name__,
                                                                                      arg_type))
                elif arg_type == 'str':
    ...
    def get_resource(self, uuid=None):
        ...
        if uuid is not None:
            ...
            instance = self.controller.api_client.admin_request('resource', uri, 'get', data='')\
                .get('applied_customization')
    ...
    def list_resources(self, customization_types=[], uuids=[], tags=[], page=0, size=-1):
        ...
        instances = self.controller.api_client.admin_request('resource', uri, 'get',
                                                             data=urlencode(data)).get('applied_customization', [])
