...
class DescribeKeyPairItemResponseSchema(Schema):
    ...
    keyName = fields.String(
        required=True,
        allow_none=True,
        example='', description='The name of the key pair')
...
class DescribeKeyPairResponseSchema(Schema):
    ...
    requestId = fields.String(
        ...
        example='',
        description='')
    keySet = fields.Nested(
        DescribeKeyPairItemResponseSchema,
        many=True,
        required=False)
...
class DescribeKeyPairsResponseSchema(Schema):
    DescribeKeyPairsResponse = fields.Nested(
        DescribeKeyPairResponseSchema,
        required=True,
        many=False,
        allow_none=False)
...
class DescribeKeyPairsRequestSchema(Schema):
    key_name_N = fields.List(fields.String(example=''), required=False, allow_none=True, context='query',
                             collection_format='multi', data_key='key-name.N', description='keypair name')
    keyName_N = fields.List(fields.String(example=''), required=False, allow_none=True, context='query',
                            collection_format='multi', data_key='KeyName.N', description='keypair name')
    ...
    Nvl_MaxResults = fields.Integer(required=False, default=10, description='', context='query',
                                    data_key='Nvl-MaxResults')
    Nvl_NextToken = fields.String(required=False, default='0', description='', context='query',
...
class DescribeKeyPairs(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': DescribeKeyPairsResponseSchema
        }
    })
    ...
    def get(self, controller, data, *args, **kwargs):
        ...
        keys, total = controller.get_service_type_plugins(service_name_list=instance_ids,
                                                          account_id_list=account_id_list,
                                                          plugintype=ApiComputeKeyPairs.plugintype,
                                                          page=page, size=size, active=True)
...
class ExportKeyPairItemResponseSchema(Schema):
    nvl_keyId = fields.String(required=True, allow_none=True, example='123', data_key='nvl-keyId', description='The id of the key pair')
    keyName = fields.String(required=True, allow_none=True, example='', description='The name of the key pair')
    keyFingerprint = fields.String(required=True, allow_none=True, example='',
        description='If you used CreateKeyPair to create the key pair, this is the SHA-1 '
        'digest of the DER encoded private key. If you used ImportKeyPair to '
        'provide AWS the public key, this is the MD5 public key fingerprint '
        'as specified in section 4 of RFC4716. ')
    nvl_ownerAlias = fields.String(required=False, allow_none=True, example='', description='name of the account that owns the key', data_key='nvl-ownerAlias')
    nvl_ownerId = fields.String(required=False, allow_none=True, example='', description='ID of the account that owns the key', data_key='nvl-ownerId')
...
class ExportKeyPairs(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ExportKeyPairsResponseSchema
        }
    })
...
class CreateKeyPairParamRequestSchema(Schema):
    ...
    KeyName = fields.String(required=True, example='ssh-key1', validate=Length(min=8, max=100),
                            description='name for the key pair')
    Nvl_KeyPairType = fields.String(required=False, missing=None, description='Key pair type definition',
                                    data_key='Nvl-KeyPairType', validate=OneOf(['DefaultKeyPair', 'KeyPair.Private']))
...
class CreateKeyPairApiResponse1Schema(Schema):
    ...
    keyName = fields.String(required=False, allow_none=True, example='my-key-name',
                            description='name of the key pair')
    keyFingerprint = fields.String(required=False, allow_none=True, example='',
                                   description='The SHA-1 digest of the DER encoded private key')
    keyMaterial = fields.String(required=False, allow_none=True, example='',
...
class CreateKeyPair(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        201: {
            'description': 'success',
            'schema': CreateKeyPairApiResponseSchema
        }
    })
    ...
    def post(self, controller: ServiceController, data: dict, *args, **kwargs):
        ...
        inst: ApiComputeKeyPairs = controller.add_service_type_plugin(service_definition_id, account.oid, name=name, desc=desc,
...
class DeleteKeyPairResponseSchema(Schema):
    DeleteKeyPairResponse = fields.Nested(
        DeleteKeyPairResponse1Schema,
        required=True,
        allow_none=False)
...
class DeleteKeyPairRequestSchema (Schema):
    ...
    # description='account id or uuid associated to compute zone')
    Nvl_KeyPairId = fields.String(required=False, example='1', data_key='Nvl-KeyPairId',
...
class DeleteKeyPair(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        202: {
            'description': 'success',
            'schema': DeleteKeyPairResponseSchema
        }
    })
...
class ImportKeyPair(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ImportKeyPairResponseSchema
        }
    })
    ...
    def post(self, controller, data, *args, **kwargs):
        ...
        instance = controller.add_service_type_plugin(service_definition_id, account.oid, name=name, desc=desc,
                                                      parent_plugin=parent_plugin, instance_config=data, action=action,
...
class ComputeKeyPairAPI(ApiView):
    ...
    def register_api(module, rules=None, **kwargs):
        base = 'nws'
        rules = [
            ('%s/computeservices/keypair/describekeypairs' % base, 'GET', DescribeKeyPairs, {}),
            ('%s/computeservices/keypair/importkeypair' % base, 'POST', ImportKeyPair, {}),
            ('%s/computeservices/keypair/createkeypair' % base, 'POST', CreateKeyPair, {}),
            ('%s/computeservices/keypair/deletekeypair' % base, 'DELETE', DeleteKeyPair, {}),
