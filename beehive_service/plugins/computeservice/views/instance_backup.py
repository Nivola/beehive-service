...
class DescribeInstanceBackup2ResponseSchema(Schema):
    ...
    hypervisor = fields.String(
        required=False,
        description="restore point hypervisor. Only openstack is actually " "supported",
    )
    site = fields.String(required=False, description="restore point site")
    resource_type = fields.String(
        required=False,
        description="restore point type of managed resource. Only " "ComputeInstance is actually supported",
    )
...
class DescribeBackupRestorePoints(ServiceApiView):
    ...
    def get(self, controller, data, *args, **kwargs):
        ...
        if job_id is not None:
            ...
            # filter for vm
            apiComputeInstance: ApiComputeInstance = controller.get_service_type_plugin(instance_id)
            restore_points, restore_point_total = apiComputeInstance.get_job_restore_points(data_search)
            restore_point_total = restore_point_total  # len(restore_points)
...
class DescribeInstanceBackupRestores1ResponseSchema(Schema):
    ...
    restoreSet = fields.Nested(
        DescribeInstanceBackupRestores2ResponseSchema,
        required=True,
        many=True,
        allow_none=True,
    )
...
class CreateBackupRestoreApiRequestSchema(Schema):
    ...
    InstanceId = fields.String(
        required=True,
        data_key="InstanceId",
        description="id of the instance to restore",
    )
    RestorePointId = fields.String(
        required=True,
        data_key="RestorePointId",
...
class DescribeBackupJobsInstanceResponseSchema(Schema):
    instanceId = fields.String(
        required=True,
        example="4ee5557c-1994-49de-8411-0652df230ef6",
...
class InstanceSetResponseSchema(Schema):
    uuid = fields.String(
        required=True,
        example="2ddd528d-c9b4-4d7e-8722-cc395140255a",
...
class DescribeBackupJobs2ResponseSchema(Schema):
    ...
    availabilityZone = fields.String(
        ...
        description="availability zone of the job",
    )
    ...
    created = fields.DateTime(
        required=True,
        example="2021-06-28T12:12:02.000000",
        description="job creation time",
    )
    updated = fields.DateTime(
        required=True,
        example="2021-07-12T15:11:49.000000",
...
class DescribeBackupJobsResponseSchema(Schema):
    DescribeBackupJobsResponse = fields.Nested(
        DescribeBackupJobs1ResponseSchema, required=True, many=False, allow_none=False
    )
...
class CreateBackupJobRequestSchema(Schema):
    ...
    Policy = fields.String(
        required=False,
        missing="bk-job-policy-7-7-retention",
        example="bk-job-policy-7-7-retention",
...
class ModifyBackupJobRequestSchema(Schema):
    ...
    Policy = fields.String(
        required=False,
        missing=None,
        example="bk-job-policy-14-retention",
        description="job policy",
    )
...
class ModifyBackupJobResponseSchema(Schema):
    ModifyBackupJob = fields.Nested(
        ModifyInstancesBackup1ResponseSchema,
        required=True,
        many=False,
        allow_none=False,
    )
...
class AddBackupJobInstanceResponseSchema(Schema):
    AddBackupJobInstance = fields.Nested(
        AddBackupJobInstance1ResponseSchema, required=True, many=False, allow_none=False
    )
...
class DelBackupJobInstanceResponseSchema(Schema):
    DelBackupJobInstance = fields.Nested(
        DelBackupJobInstance1ResponseSchema, required=True, many=False, allow_none=False
    )
...
class JobPolicyResponseSchema(Schema):
    ...
    start_time_window = fields.String(
        required=True,
        example="19:00-05:00",
        description="window where is selected " "starting tim from the system",
    )
    interval = fields.String(
        required=True,
        example="24hrs",
        description="time interval from two restore points",
    )
