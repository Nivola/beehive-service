...
-- -----------------------------------------------------------
-- -----------------------------------------------------------
DROP PROCEDURE IF EXISTS compute_daily_consumes;

DELIMITER $$

CREATE PROCEDURE compute_daily_consumes(
...
BEGIN
    ...
    IF smetrics = 0 THEN
        ...
        SELECT CONCAT( 'COMPUTING DAILY CONSUME FOR ACCOUNT ', p_account_id, ' AT ', p_period, 'with task ', jobid ) as MSG;
    ...
    ELSE
        SELECT CONCAT( 'SKIPING ACCOUNT: FOUND ', smetrics, ' DAILY CONSUME FOR ACCOUNT ', p_account_id, ' AT ', p_period )as MSG;
