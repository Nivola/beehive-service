...
-- mysql -u root --password=xxx service

CREATE OR REPLACE
ALGORITHM = UNDEFINED VIEW `service`.`v_service_instant_consume` AS 
with vms as (
select
    ...
    1 AS `fk_job_id`
from
    (((`service`.`service_instance` `si`
join `service`.`service_definition` `sd` on
    (`si`.`fk_service_definition_id` = `sd`.`id`))
join `service`.`service_type` `st` on
    (`sd`.`fk_service_type_id` = `st`.`id`))
join `service`.`service_plugin_type` `pt` on
    (`st`.`objclass` = `pt`.`objclass`))
where
    `si`.`active` = 1
...
select
    ...
    NULL AS `expiry_date`,
    min(`si`.`id`) AS `id`,
    'MonitoringService' AS `plugin_name`,
    'monit_numero_vm_tot' AS `metric_group_name`,
    count(`si`.`id`) AS `metric_instant_value`,
    '#' AS `metric_unit`,
    count(`si`.`id`) AS `metric_value`,
    min(`si`.`id`) AS `fk_service_instance_id`,
...
where
    `si`.`active` = 1
    and `pt`.`name_type` = 'MonitoringInstance'
group by
    `si`.`fk_account_id`),
dbs as (
select
...
join `service`.`service_instance` `si` on
    (`ac`.`fk_service_instance_id` = `si`.`id`))
join `service`.`service_definition` `sd` on
    (`si`.`fk_service_definition_id` = `sd`.`id`))
join `service`.`service_type` `st` on
    (`sd`.`fk_service_type_id` = `st`.`id`))
join `service`.`service_plugin_type` `pt` on
    (`st`.`objclass` = `pt`.`objclass`))
join `service`.`service_plugin_type` `pt2` on
    (`pt`.`service_category` = `pt2`.`service_category`
        and `pt2`.`category` = 'CONTAINER'))
left join `service`.`service_metric_type` `me` on
    (`ac`.`fk_metric_type_id` = `me`.`id`))
where
...
union all
select
    `monits`.`creation_date` AS `creation_date`,
    `monits`.`modification_date` AS `modification_date`,
    `monits`.`expiry_date` AS `expiry_date`,
    `monits`.`id` AS `id`,
    `monits`.`plugin_name` AS `plugin_name`,
    `monits`.`metric_group_name` AS `metric_group_name`,
    `monits`.`metric_instant_value` AS `metric_instant_value`,
    `monits`.`metric_unit` AS `metric_unit`,
    `monits`.`metric_value` AS `metric_value`,
    `monits`.`fk_service_instance_id` AS `fk_service_instance_id`,
    `monits`.`fk_account_id` AS `fk_account_id`,
    `monits`.`fk_job_id` AS `fk_job_id`
from
    `monits`
union all
select
