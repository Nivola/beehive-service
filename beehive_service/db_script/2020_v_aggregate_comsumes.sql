...
-- service_stage.v_aggregate_consumes source

CREATE OR REPLACEVIEW v_aggregate_consumes AS
with recursive tree as (
    select
        t1.start_service_id AS parent_id, t1.start_service_id AS child_id, 0 AS dep
    from
        (service_link_inst t1
    left join service_link_inst t2 on
        (t1.start_service_id = t2.end_service_id))
    where
        t2.id is null
    group by
        t1.start_service_id
    ...
    from
        (tree c
    join service_link_inst t2 on
        (t2.start_service_id = c.child_id))),
pinfo as (
    select
        straight_join pt.name_type AS name_type, pt.category AS category, st.name AS instance_type, si.id AS fk_service_id, si.uuid AS service_uuid, a.uuid AS account_uuid, d.uuid AS division_uuid, o.uuid AS organization_uuid
    from
        ((((((service_instance si
