...
from beehive.common.data import get_operation_params, set_operation_params, operation, trace
...
class ApiServiceCatalog(ServiceApiObject):
    ...
    role_templates = {
        'master': {
            ...
            'perms': [
                {'subsystem': 'service', 'type': 'ServiceCatalog', 'objid': '<objid>', 'action': '*'},
            ],
            'perm_tmpls': [
                {'subsystem': 'service', 'type': 'ServiceType.ServiceDefinition', 'objid': '<objid>', 'action': 'view'}
            ]
        },
        'viewer': {
            ...
            'perms': [
                {'subsystem': 'service', 'type': 'ServiceCatalog', 'objid': '<objid>', 'action': 'view'}
            ]
        },
        'operator': {
    ...
    def get_service_definitions(self):
        ...
        res = [
            ...
            for r in srv_defs]
        self.logger.debug(
            'Get service definitions linked to catalog %s: %s' %
            (self.oid, res))
        return res
    ...
    def get_paginated_service_defs(self, *args, **kvargs):
        ...
        res, total = self.controller.get_paginated_entities(
            ApiServiceDefinition, recuperaServiceDefinitions, customize=customize, authorize=False, *args, **kvargs)
    ...
    def __set_perms_objid(self, perms, objid):
        ...
        for perm in perms:
            if perm.get('objid').find('<objid>') >= 0:
                ...
                new_perm['objid'] = new_perm['objid'].replace(
                    '<objid>', objid)
    ...
    def get_users(self):
        ...
        try:
            ...
            self.logger.error(
                'Error get %s %s users' %
                (self.objname, self.name), exc_info=1)
            raise ApiManagerError(
                'Error get %s %s users' %
                (self.objname, self.name))
        self.logger.debug(
            'Get %s %s users: %s' %
            (self.objname, self.name, truncate(str(users))))
        return res
