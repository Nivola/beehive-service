...
class UpdateServiceInstance(ServiceApiView):
    ...
    def put(self, controller, data, oid, *args, **kvargs):
        ...
        # codice morto ma proma era cosi pluginCh = srv_inst.instancePlugin(oid)
...
class DeleteServiceInstance(ServiceApiView):
    ...
    def delete(self, controller, data, oid, *args, **kvargs):
        ...
        controller.check_authorization(srv_inst.objtype, srv_inst.objdef, srv_inst.objid, action="delete")

        if srv_inst.model.linkChildren is not None and srv_inst.model.linkChildren.count() > 0 and not recursive_delete:
            self.logger.warn(
...
class ListServiceInstanceLinksResponseSchema(Schema):
    links = fields.Nested(
        ListServiceInstanceLinksParamsResponseSchema,
        many=True,
        required=True,
        allow_none=True,
    )
...
class ServiceUserTasksList(ServiceApiView):
    ...
    parameters = SwaggerHelper().get_parameters(GetApiObjectRequestSchema)
    responses = ServiceApiView.setResponses({200: {"description": "success", "schema": ServiceTaskListResponseSchema}})
...
class ServiceTaskDetailRequestSchema(GetApiObjectRequestSchema):
    taskid = fields.String(required=True, description="attribute name", context="path")
...
class GetServiceTasksDetail(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses(
        {200: {"description": "success", "schema": ServiceTaskDetaiLResponseSchema}}
    )
...
class ServiceInstanceAPI(ApiView):
    ...
    def register_api(module, dummyrules=None, version=None):
        ...
        rules = [
            ...
            (
                "%s/serviceinsts/<oid>/task/<taskid>" % base,
                "GET",
                GetServiceTasksDetail,
                {},
            ),
            (
                "%s/serviceinsts/<oid>/task/<taskid>" % base,
                "POST",
                SetServiceTasksDetail,
                {},
            ),
        ]
