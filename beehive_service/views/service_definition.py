...
from beehive.common.apimanager import PaginatedRequestQuerySchema, \
    ...
    SwaggerApiView, ApiView, ApiManagerWarning, ApiManagerError
from flasgger import fields, Schema
from beecell.swagger import SwaggerHelper
from beehive_service.entity.service_definition import ApiServiceLinkDef, ApiServiceConfig, ApiServiceDefinition
...
class GetServiceDefinitionResponseSchema(Schema):
    servicedef = fields.Nested(GetServiceDefinitionParamsResponseSchema, required=True, many=True, allow_none=True)
...
class GetServiceDefinition(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetServiceDefinitionResponseSchema
        }
    })
...
class ListServiceDefinitionRequestSchema(ApiServiceObjectRequestSchema, ApiObjectRequestFiltersSchema,
...
class ListServiceDefinition(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ListServiceDefinitionResponseSchema
        }
    })
...
class UpdateServiceDefinitionParamRequestSchema(Schema):
    ...
    config = fields.String(required=False, allow_none=True, many=False,
...
class DeleteServiceDefinition(ServiceApiView):
    ...
    def delete(self, controller, data, oid, *args, **kvargs):
        # recursive_delete = data.pop('recursive')
        srv_def = controller.get_service_def(oid)
        #
        # # make soft delete of: ServiceDefinition, Servicelink, ServiceConfig entity
        # entity = kvargs.get('definition')
        # if entity is not None and entity.oid == oid:  # in recursive call
        #     srv_def = entity
        # else:
        #     srv_def = controller.get_service_def(oid)
        #
        # controller.check_authorization(srv_def.objtype, srv_def.objdef, srv_def.objid, action='delete')
        #
        # if srv_def.model.linkChildren is not None and srv_def.model.linkChildren.count() > 0:
        #     if not recursive_delete:
        #         raise ApiManagerWarning('This instance has %s children. Forze deletion with parameter \'recursive\' '
        #                                 'to true.' % srv_def.oid)
        #
        # # delete definition config
        # for cfg in srv_def.model.config_params:
        #     self.logger.warning('Delete ServiceDefinitionConfig: %s' % cfg)
        #     api_cfg = ServiceUtil.instanceApi(controller, ApiServiceConfig, cfg)
        #     api_cfg.delete(soft=True)
        #
        # # delete definition link Parent
        # for l in srv_def.model.linkParent:
        #     self.logger.warning('Delete ServiceDefinitionConfig: %s' % l)
        #     api_link = ServiceUtil.instanceApi(controller, ApiServiceLinkDef, l)
        #     api_link.delete(soft=True)
        #
        # # delete definition link and children
        # for l in srv_def.model.linkChildren:
        #     self.logger.warning('Delete ServiceDefinitionConfig: %s' % l)
        #     # api_link = ServiceUtil.instanceApi(controller, ApiServiceLinkDef, l)
        #     child = ServiceUtil.instanceApi(controller, ApiServiceDefinition, l.end_service)
        #     self.delete(controller, data, l.end_service_id, definition=child)
        #
...
#
class ListServiceConfigsRequestSchema(ApiServiceObjectRequestSchema, ApiObjectRequestFiltersSchema,
                                      PaginatedRequestQuerySchema):
