...
from beehive.common.apimanager import ApiObjectResponseDateSchema, ApiView, PaginatedRequestQuerySchema, \
...
from beehive_service.views.account import ListAccountsResponseSchema,\
    ContainerInstancesItemResponseSchema


class ListDivisionsRequestSchema(ApiServiceObjectRequestSchema, ApiObjectRequestFiltersSchema,
...
class ListDivisionsResponseSchema(PaginatedResponseSchema):
    divisions = fields.Nested(
        ListDivisionsParamsResponseSchema, many=True, required=True, allow_none=True)
...
class ListDivisions(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ListDivisionsResponseSchema
        }
    })
    ...
    def get(self, controller: ServiceController, data, *args, **kwargs):
        ...
        for r in divisions:
            ...
            try:
                info['organization_name'] = getattr(
                    orgs[str(r.organization_id)], 'name')
            except:
                self.logger.warning('organization_id=%s not found in organization_idx list=%s' % (
                    r.organization_id, orgs))
...
class GetDivisionResponseSchema(Schema):
    division = fields.Nested(
        GetDivisionParamsResponseSchema, required=True, allow_none=True)
...
class GetDivision(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetDivisionResponseSchema
        }
    })
...
class GetDivisionAccounts(ServiceApiView):
    ...
    def get(self, controller, data, oid, *args, **kwargs):
        accounts, total = controller.get_accounts(
            division_id=controller.get_division(oid).oid)
...
class CreateDivisionParamRequestSchema(Schema):
    ...
    email = fields.String(required=False, allow_none=True)
    postaladdress = fields.String(required=False, allow_none=True)
    price_list_id = fields.String(required=False, allow_none=True,description='DEPRECATED Price List are not managed any more')
...
class GetDivisionRolesResponseSchema(Schema):
    roles = fields.Nested(GetDivisionRolesItemResponseSchema,
...
class GetDivisionRoles(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetDivisionRolesResponseSchema
        }
    })
...
class GetDivisionUsersResponseSchema(Schema):
    users = fields.Nested(GetDivisionUsersItemResponseSchema,
                          required=True, many=True, allow_none=True)
    count = fields.Integer(required=True)
...
class GetDivisionUsers(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetDivisionUsersResponseSchema
        }
    })
...
class SetDivisionUsersParamRequestSchema(Schema):
    user_id = fields.String(required=False, default='prova',
...
class GetDivisionGroupsResponseSchema(Schema):
    groups = fields.Nested(GetDivisionGroupsItemResponseSchema,
                           required=True, many=True, allow_none=True)
...
class GetDivisionGroups(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetDivisionGroupsResponseSchema
        }
    })
...
class SetDivisionGroupsParamRequestSchema(Schema):
    group_id = fields.String(
        required=False, default='prova', description='Group name, id or uuid')
    role = fields.String(required=False, default='prova', description='Role name, id or uuid',
                         validate=OneOf(ApiDivision.role_templates.keys()))
...
class UnsetDivisionGroups(ServiceApiView):
    ...
    parameters = SwaggerHelper().get_parameters(
        UnsetDivisionGroupsBodyRequestSchema)
...
class GetActiveServicesByDivisionResponse1Schema (Schema):
    service_container = fields.Nested(
        ContainerInstancesItemResponseSchema, many=True, required=True, allow_none=False)
...
class GetActiveServicesByDivisionResponseSchema (Schema):
    services = fields.Nested(GetActiveServicesByDivisionResponse1Schema,
                             required=True, many=False, allow_none=False)


class GetActiveServicesByDivision(ServiceApiView):
    summary = 'Returns the active services list for an division, for each service are provided information about ' \
              'resources usage'
    description = 'Returns the active services list for an division, for each service are provided information ' \
    ...
    parameters = SwaggerHelper().get_parameters(
        GetActiveServicesByDivisionApiRequestSchema)
    ...
    def get(self, controller, data, oid, *args, **kwargs):
        ...
        active_services = controller.get_service_instant_consume_by_division(
            division.oid)
...
class DivisionAPI(ApiView):
    ...
    def register_api(module, rules=None, **kwargs):
        ...
        rules = [
            ...
            ('%s/divisions/<oid>/accounts' %
             base, 'GET', GetDivisionAccounts, {}),
            ...
            ('%s/divisions/<oid>/users' %
             base, 'DELETE', UnsetDivisionUsers, {}),
            ('%s/divisions/<oid>/groups' % base, 'GET', GetDivisionGroups, {}),
            ('%s/divisions/<oid>/groups' % base, 'POST', SetDivisionGroups, {}),
            ('%s/divisions/<oid>/groups' %
             base, 'DELETE', UnsetDivisionGroups, {}),
            ('%s/divisions/<oid>/activeservices' %
             base, 'GET', GetActiveServicesByDivision, {}),
        ]
