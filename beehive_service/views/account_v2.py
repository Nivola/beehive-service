...
from beehive.common.apimanager import ApiObjectMetadataResponseSchema, ApiObjectResponseDateSchema, ApiView, PaginatedRequestQuerySchema, \
...
class ListAccountsV20RequestSchema(ApiServiceObjectRequestSchema, ApiObjectRequestFiltersSchema,
...
class ListAccountsV20(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ListAccountsV20ResponseSchema
        }
    })
...
class GetAccountV20(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetAccountV20ResponseSchema
        }
    })
...
class DeleteAccountV20RequestSchema(Schema):
    delete_services = fields.Boolean(required=False, missing=False, example=False, context='query',
                                     description='if True delete all child services before remove the account')
    delete_tags = fields.Boolean(required=False, missing=False, example=False, context='query',
...
class GetAccountDefinitionsRequestSchema(ApiServiceObjectRequestSchema, ApiObjectRequestFiltersSchema,
                                         PaginatedRequestQuerySchema):
    ...
    category = fields.String(required=False, context='query', description='definiton category',
                             validate=OneOf(__SRV_SERVICE_CATEGORY__))
    only_container = fields.Boolean(required=False, context='query',
...
class GetAccountDefinitions(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetAccountDefinitionsResponseSchema
        }
    })
...
class AddAccountDefinitions(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': AddAccountDefinitionsResponseSchema
        }
    })
    ...
    def post(self, controller: ServiceController, data: dict, oid: str, *args, **kwargs):
        ...
        for definition in definitions:
            try:
                ...
                controller.add_account_service_definition(account.oid, apidefinition.oid, account=account,
                                                          servicedefinition=apidefinition)
                res.append(apidefinition.uuid)
            except Exception as ex:
                self.logger.warning('error adding service definition %s in account %s: %s' %
                                    (apidefinition.oid, account.oid, ex))
                pass
