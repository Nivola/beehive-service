...
from beehive_service.views import ServiceApiView, \
    ApiObjectRequestFiltersSchema, ApiServiceObjectRequestSchema
from beehive_service.views.account import ContainerInstancesItemResponseSchema


class ListOrganizationsRequestSchema(ApiServiceObjectRequestSchema, ApiObjectRequestFiltersSchema,
...
class GetOrganizationRoles(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetOrganizationRolesResponseSchema
        }
    })
...
class GetOrganizationUsers(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetOrganizationUsersResponseSchema
        }
    })
...
class UnsetOrganizationUsersParamRequestSchema(Schema):
    user_id = fields.String(required=True, default='prova', description='User name, id or uuid',allow_none=False,validate=Length(1, error='user_id Must not be Empty'))
    role = fields.String(required=False, default='prova', description='Role name, id or uuid',
                         validate=OneOf(ApiOrganization.role_templates.keys()))
...
class GetOrganizationGroups(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetOrganizationGroupsResponseSchema
        }
    })
...
class GetActiveServicesByOrganizationResponseSchema (Schema):
    services = fields.Nested(GetActiveServicesByOrganitazionResponse1Schema, required=True, many=False, allow_none=False)


class GetActiveServicesByOrganization(ServiceApiView):
    summary = 'Returns the active services list for an organization, for each service are provided information about ' \
              'resources usage'
    description = 'Returns the active services list for an organization, for each service are provided information ' \
...
class OrganizationAPI(ApiView):
    ...
    def register_api(module, rules=None, **kwargs):
        ...
        rules = [
            ...
            ('%s/organizations/<oid>/users' % base, 'DELETE', UnsetOrganizationUsers, {}),
            ('%s/organizations/<oid>/groups' % base, 'GET', GetOrganizationGroups, {}),
            ('%s/organizations/<oid>/groups' % base, 'POST', SetOrganizationGroups, {}),
            ('%s/organizations/<oid>/groups' % base, 'DELETE', UnsetOrganizationGroups, {}),
            ('%s/organizations/<oid>/activeservices' % base, 'GET', GetActiveServicesByOrganization, {}),
        ]
