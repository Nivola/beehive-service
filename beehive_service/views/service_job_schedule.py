...
class GetServiceScheduleResponseSchema(Schema):
    job_schedule = fields.Nested(GetServiceScheduleParamsResponseSchema,
...
class GetServiceSchedule(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': GetServiceScheduleResponseSchema
        }
    })
...
## list
class ListServiceScheduleRequestSchema(ApiServiceObjectRequestSchema,
                                   ApiObjectRequestFiltersSchema,
                                   PaginatedRequestQuerySchema):
    job_name = fields.String(Required=False, context='query')
    schedule_type = fields.String(Required=False, context='query',
                                  validate=OneOf(__SCHEDULE_TYPE__))
...
class ListServiceSchedule(ServiceApiView):
    ...
    responses = SwaggerApiView.setResponses({
        200: {
            'description': 'success',
            'schema': ListServiceScheduleResponseSchema
        }
    })
        ...
        if __SCHEDULE_TYPE__[0] == schedule_type : # crontab
            ...
            for key in keys:
                if key in ['days', 'seconds', 'minutes', 'hours', 'weeks']:
                    raise ValidationError('For schedule type %s the parameter is not supported : %s' %(schedule_type, key))
        elif __SCHEDULE_TYPE__[1] == schedule_type : # timedelta
            keys = data.get('schedule_params', {})
            for key in keys:
                if key in ['minute', 'hour', 'day_of_week', 'day_of_month', 'month_of_year']:
                    raise ValidationError('For schedule type %s the parameter is not supported : %s' %(schedule_type, key))
...
class CreateServiceScheduleParamRequestSchema(ApiBaseServiceObjectCreateRequestSchema):
    ...
    # partial = fields.Raw(required=False, allow_none=True )


class CreateServiceScheduleRequestSchema(Schema):
    job_schedule = fields.Nested(CreateServiceScheduleParamRequestSchema,
                                 context='body')
