...
__REGEX_PORT_RANGE__ = '^((6553[0-5])|(655[0-2][0-9])|(65[0-4][0-9]{2})|(6[0-4][0-9]{3})|' \
...
__SRV_AWS_STORAGE_STATUS__ = ['creating', 'available', 'deleting', 'deleted','unknown','error']
...
__SRV_PORTAL_FILTER_SERVICE_OBJECT_TYPE__ = ['organization', 'division', 'account', 'catalog']
...
class ServiceUtil(object):
    ...
    def instance_api(controller, api_class, model):
        ...
        if isinstance(model, list):
            for e in model:
                api = api_class(controller, oid=e.id, objid=e.objid, name=e.name, desc=e.desc, active=e.active,
                                model=e)
                api_list.append(api)
            return api_list
        else:
            ...
            if model is not None:
                api = api_class(controller, oid=model.id, objid=model.objid, name=model.name, desc=model.desc,
                                active=model.active, model=model)
            return api
    ...
    def instanceApi(controller, apiClass, entity):
        ...
        if isinstance(entity, list):
            for e in entity:
                api = apiClass(controller, oid=e.id,
                               objid=e.objid,
                               name=e.name,
                               desc=e.desc,
                               active=e.active,
            ...
            if entity is not None:
                api = apiClass(controller, oid=entity.id,
                               objid=entity.objid,
                               name=entity.name,
                               desc=entity.desc,
    ...
    def creation_in_progress(instance):
        ...
        return (instance.status == SrvStatusType.DRAFT and instance.bpmn_process_id is not None) \
            or (instance.status == SrvStatusType.PENDING)
    ...
    def is_draft(instance):
        ...
        """
        AssertUtil.assert_is_not_none(instance)
        return instance.status == SrvStatusType.DRAFT \
            and not ServiceUtil.creation_in_progress(instance)
    ...
    def is_visible(instance):
        ...
        """
        AssertUtil.assert_is_not_none(instance)
        return ServiceUtil.check_status(instance, SrvStatusType.ACTIVE) \
            or ServiceUtil.check_status(instance, SrvStatusType.STOPPED)
    ...
    def is_active(instance):
        ...
        """
        AssertUtil.assert_is_not_none(instance)
        return instance.status in [SrvStatusType.ACTIVE,
